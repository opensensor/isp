...
INTP_KEY = "fn_000000000005005c|tisp_dmsc_intp|5005c|1890"
...
def parse_args():
    ...
    parser.add_argument(
        "--layout-output", type=Path,
        help="write the recovered DMSC field layout as a C macro",
    )
    parser.add_argument(
        "--interpolation-output", type=Path,
        help="write the OEM DMSC interpolation assignments as a C macro",
    )
    parser.add_argument(
        "--ref-writes-output", type=Path,
        help="write the OEM gain-dependent DMSC register writer as a C macro",
    )
    parser.add_argument(
        "--noref-writes-output", type=Path,
        help="write the OEM fixed DMSC register writer as a C macro",
    )
...
def render_layout(layout):
    lines = [
        ...
        "#define REGTRACE_T23_DMSC_TUNING_FIELDS(X) \\",
    ]
    offset = 0
    for index, (name, size) in enumerate(layout):
        suffix = " \\" if index + 1 != len(layout) else ""
        lines.append(
            f"    X({name}, 0x{offset:04x}U, 0x{size:x}U){suffix}"
        )
        offset += size
    return "\n".join(lines) + "\n"


def parse_interpolation_fields(text):
    fields = re.findall(
        r"\b(dmsc_[A-Za-z0-9_]+_intp)\s*=\s*"
        r"tisp_simple_intp\([^,]+,\s*[^,]+,\s*&"
        r"(dmsc_[A-Za-z0-9_]+_array)\)",
        text,
    )
    if len(fields) != 222 or len(set(fields)) != len(fields):
        raise ValueError(
            f"unexpected DMSC interpolation field count: {len(fields)}"
        )
    return fields
...
def render_interpolation(fields):
    lines = [
        "/* Generated by tools/t23_dmsc_image.py. */",
        "#define REGTRACE_T23_DMSC_INTERPOLATED_FIELDS(X) \\",
    ]
    for index, (destination, curve) in enumerate(fields):
        suffix = " \\" if index + 1 != len(fields) else ""
        lines.append(f"    X({destination}, {curve}){suffix}")
    return "\n".join(lines) + "\n"


def alias_reference(fields, symbols, alias):
    ...
    matches = [
        ...
        if address <= raw_address < address + size and name in fields
    ]
    if not matches:
...
def render_runtime_expression(expression, fields, symbols):
    placeholders = {}

    def field_word(name, offset, width=4):
        key = f"__REGTRACE_DMSC_FIELD_{len(placeholders)}__"
        value = (
            f"regtrace_t23_dmsc_field_word(&({name}), 0x{offset:x}U)"
        )
        if width == 2:
            value = f"({value} & 0xffffU)"
        elif width == 1:
            value = f"({value} & 0xffU)"
        placeholders[key] = value
        return key

    expression = expression.strip().replace("$a1_139", "a1_139")
    expression = expression.replace("zx.d(", "(")
    expression = re.sub(
        r"\b(dmsc_[A-Za-z0-9_]+):(0x[0-9a-fA-F]+|[0-9]+)(?:\.([dwb]))?",
        lambda match: field_word(
            match.group(1),
            int(match.group(2), 0),
            {"d": 4, "w": 2, "b": 1}.get(match.group(3), 4),
        ),
        expression,
    )
    expression = re.sub(
        r"\b(dmsc_[A-Za-z0-9_]+)\.d",
        lambda match: field_word(match.group(1), 0),
        expression,
    )
    expression = re.sub(
        r"\b(data_[0-9a-f]+)\b",
        lambda match: field_word(
            *alias_reference(fields, symbols, match.group(1))
        ),
        expression,
    )
...
def render_ref_writes(
    text, fields, symbols,
    macro_name="REGTRACE_T23_DMSC_REF_REG_CFG_BODY",
    expected_writes=100,
):
    lines = [
        "/* Generated by tools/t23_dmsc_image.py. */",
        f"#define {macro_name}() do {{ \\",
    ]
    if "$a1_139" in text:
        lines.append("    uint32_t a1_139; \\")
    write_count = 0
    for raw_line in text.splitlines():
        line = raw_line.strip()
        temporary = re.match(r"uint32_t \$a1_139 = (.+)", line)
        if temporary:
            expression = render_runtime_expression(
                temporary.group(1), fields, symbols
            )
            lines.append(f"    a1_139 = {expression}; \\")
            continue

        marker = "system_reg_write("
        if marker not in line:
            continue
        tailcall = line.startswith("return ")
        ...
        register = evaluate_register(register_expression)
        value = render_runtime_expression(value_expression, fields, symbols)
        prefix = "return " if tailcall else ""
        lines.append(
            f"    {prefix}system_reg_write(0x{register:04x}U, {value}); \\"
        )
        write_count += 1

    if write_count != expected_writes or not lines[-1].startswith("    return "):
        raise ValueError(f"unexpected DMSC ref writer: writes={write_count}")
...
def main():
    ...
    if args.layout_output:
        args.layout_output.write_text(render_layout(layout), encoding="ascii")
    if args.interpolation_output:
        interpolation = parse_interpolation_fields(
            functions[INTP_KEY]["decompiled_text"]
        )
        args.interpolation_output.write_text(
            render_interpolation(interpolation), encoding="ascii"
        )
    image, fields = load_fields(args.tuning_bin, layout)
    symbols = parse_symbols(args.original_ko)
    if args.noref_writes_output:
        args.noref_writes_output.write_text(
            render_ref_writes(
                functions[NOREF_KEY]["decompiled_text"],
                fields,
                symbols,
                macro_name="REGTRACE_T23_DMSC_NOREF_REG_CFG_BODY",
                expected_writes=28,
            ),
            encoding="ascii",
        )
    if args.ref_writes_output:
        args.ref_writes_output.write_text(
            render_ref_writes(
                functions[REF_KEY]["decompiled_text"], fields, symbols
            ),
            encoding="ascii",
        )
