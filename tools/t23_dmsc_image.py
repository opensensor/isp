...
def parse_args():
    ...
    parser.add_argument("--base-gain-q16", type=lambda value: int(value, 0))
    parser.add_argument(
        "--symbol", default="regtrace_t23_dmsc_sc2336_startup"
    )
...
def simple_intp(fields, name, gain_q16):
    array_name = name.removesuffix("_intp") + "_array"
    gain_index = gain_q16 >> 16
    gain_fraction = gain_q16 & 0xFFFF
    if gain_index >= 8:
        return read_field(fields, array_name, 8 * 4)

    low = read_field(fields, array_name, gain_index * 4)
    high = read_field(fields, array_name, (gain_index + 1) * 4)
    delta = abs(high - low) * gain_fraction
    delta = (delta >> 16) + ((delta & 0x8000) >> 15)
    return low + delta if high >= low else low - delta


def evaluate_value(expression, fields, symbols, temporaries, gain_q16):
    ...
    def replace_intp(match):
        name = match.group(1)
        return str(simple_intp(fields, name, gain_q16))
...
def parse_writes(text, fields, symbols, gain_q16):
    writes = []
    temporaries = {}
    for raw_line in text.splitlines():
        ...
        if temporary:
            temporaries[temporary.group(1)] = evaluate_value(
                temporary.group(2), fields, symbols, temporaries, gain_q16
            )
            continue
        ...
        writes.append(
            (
                ...
                evaluate_value(
                    value_expression, fields, symbols, temporaries, gain_q16
                ),
...
def render(image, noref_writes, ref_writes, gain_q16, symbol, base_ref_writes=None):
    ...
    if base_ref_writes is not None:
        if len(base_ref_writes) != len(ref_writes):
            raise ValueError("base and target DMSC write counts differ")
        for old, new in zip(base_ref_writes, ref_writes):
            if old[0] != new[0]:
                raise ValueError(
                    f"base and target DMSC registers differ: "
                    f"0x{old[0]:x} != 0x{new[0]:x}"
                )
        changed = [
            new
            for old, new in zip(base_ref_writes, ref_writes)
        ...
        lines.append("    /* tisp_dmsc_intp_reg_refresh delta */")
        for register, value, expression in changed:
            lines.append(
        ...
        lines.extend(
            [
                "    /* tisp_dmsc_intp_reg_refresh commit */",
                "    { 0x499cU, 0x00000001U },",
                "};",
                "",
            ]
...
def main():
    ...
    noref_writes = parse_writes(
        functions[NOREF_KEY]["decompiled_text"], fields, symbols, args.gain_q16
    )
    ref_writes = parse_writes(
        functions[REF_KEY]["decompiled_text"], fields, symbols, args.gain_q16
    )
    base_ref_writes = None
    if args.base_gain_q16 is not None:
        base_ref_writes = parse_writes(
            functions[REF_KEY]["decompiled_text"],
            fields,
            symbols,
            args.base_gain_q16,
        )
