...
def render(path: Path) -> str:
    ...
    settled_ev_raw = read_words(blob, AWB_EV_OFFSET, 1)[0]
    settled_ev = settled_ev_raw >> 10
    ...
    svalue = [
        interpolate(data[name], ev_list, settled_ev)
        for name in ("smin_s", "smax_s", "smin_m", "smax_m")
    ]
    contrast = [
        data["contrast"][0],
        interpolate(data["cxl"], ev_list, settled_ev),
        interpolate(data["cxh"], ev_list, settled_ev),
        interpolate(data["cyl"], ev_list, settled_ev),
        interpolate(data["cyh"], ev_list, settled_ev),
    ]
    ...
    lines = [
        ...
        f"/* First-event EV {settled_ev_raw} >> 10 = {settled_ev}; CT {settled_ct} selects D. */",
