...
/* Frame synchronization globals - CRITICAL for frame data transfer */
static volatile int frame_done_cond = 0;
static uint64_t frame_done_cnt = 0;
static DECLARE_WAIT_QUEUE_HEAD(frame_done_wq);

/**
 * isp_frame_done_wakeup - EXACT Binary Ninja implementation
 * Wakes up processes waiting for frame completion
 */
void isp_frame_done_wakeup(void)
{
    /* Binary Ninja: frame_done_cnt_1 = frame_done_cnt.d */
    uint32_t frame_done_cnt_1 = (uint32_t)frame_done_cnt;
    
    /* Binary Ninja: $v0_1 = (frame_done_cnt_1 + 1 u< frame_done_cnt_1 ? 1 : 0) + frame_done_cnt:4 */
    uint32_t carry = (frame_done_cnt_1 + 1 < frame_done_cnt_1) ? 1 : 0;
    uint32_t high_part = (uint32_t)(frame_done_cnt >> 32) + carry;
    
    /* Binary Ninja: frame_done_cnt.d = frame_done_cnt_1 + 1 */
    frame_done_cnt = ((uint64_t)high_part << 32) | (frame_done_cnt_1 + 1);
...
/**
 * isp_frame_done_wait - EXACT Binary Ninja implementation
 * Waits for frame completion with timeout
 */
int isp_frame_done_wait(int timeout_ms, uint64_t *frame_count)
{
    ...
    if (!frame_count) {
        pr_err("isp_frame_done_wait: Invalid frame_count pointer\n");
        return -EINVAL;
    }
    ...
    if (frame_done_cond != 1) {
        while (true) {
            ...
            if (frame_done_cond != 1) {
                ...
                /* Binary Ninja: int32_t $v0_6 = schedule_timeout($s0) */
                ret = schedule_timeout(msecs_to_jiffies(timeout_ms));
                
                /* Binary Ninja: if ($v0_6 != 0) continue */
                if (ret != 0) {
                    continue;
                }
            } else if (timeout_ms != 0) {
                /* Frame done condition met */
                break;
            }
            
            /* Binary Ninja: $s0 = (frame_done_cond ^ 1) u< 1 ? 1 : 0 */
            ret = (frame_done_cond == 1) ? 1 : 0;
            break;
        }
        
        /* Binary Ninja: finish_wait(&frame_done_wq, &var_38) */
        finish_wait(&frame_done_wq, &wait);
    }
    
    /* Binary Ninja: arg2[1] = frame_done_cnt:4; *arg2 = frame_done_cnt_1 */
    *frame_count = frame_done_cnt;
...
/* Additional frame sync function needed by tuning system */
void isp_frame_sync_wakeup(void)
{
    pr_info("*** isp_frame_sync_wakeup: Frame sync event processed ***\n");
    
    /* Call the main frame done wakeup function */
    isp_frame_done_wakeup();
    
    /* Additional frame sync processing could go here */
