...
/* Forward declarations for VIC/VIC dispatcher handlers from tx-isp-module.c */
...
/* ispcore_video_s_stream - Modified for REVERSE s_stream calls */
int ispcore_video_s_stream(struct tx_isp_subdev *sd, int enable)
{
    ...
    pr_info("*** ispcore_video_s_stream: REVERSE traversal implementation - enable=%d ***\n", enable);
    ...
    /* Handle stream enable/disable state transitions */
    if (enable == 0) {
        if (v0_3 == 4) {
            /* Frame channel loop for stream OFF */
            int s2_1 = 0;
            void *v0_5;
            void *v0_6;
    ...
    /* MODIFIED: Start from the END of the subdevs array and traverse BACKWARDS */
    /* Calculate the last valid position (ISP_MAX_SUBDEVS - 1) */
    s3_1 = &isp_dev->subdevs[ISP_MAX_SUBDEVS - 1];

    pr_info("*** ispcore_video_s_stream: Starting REVERSE traversal from end of subdevs array ***\n");

    /* MODIFIED: Main subdev iteration loop - REVERSE ORDER */
    while (true) {
        ...
        if (a0_5 != NULL) {
            ...
            if (video_ops != NULL) {
                ...
                if (s_stream_func == NULL) {
                    ...
                    pr_info("*** ispcore_video_s_stream: [REVERSE] Called s_stream on subdev %s: result=%d ***\n",
        ...
        /* MODIFIED: Check if we've reached the beginning of the array */
        if (s3_1 == &isp_dev->subdevs[0]) {
            pr_info("*** ispcore_video_s_stream: Reached beginning of array, ending reverse traversal ***\n");
            break;
        }

        /* MODIFIED: Move backwards instead of forwards */
        s3_1--;
    }
...
static int tx_isp_request_irq(struct platform_device *pdev, void *irq_info)
{
    ...
    /* Select IRQ handlers based on device type (VIC uses isp_irq_handle path) */
    irq_handler_t top = tx_isp_core_irq_handle;
    irq_handler_t thread = tx_isp_core_irq_thread_handle;
    const char *name = dev_name(&pdev->dev);
    if (name && strstr(name, "isp-w02")) {
        top = isp_irq_handle;
