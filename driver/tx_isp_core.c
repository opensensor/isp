...
int tx_isp_core_start(struct tx_isp_subdev *sd)
{
    ...
    if (!sd) {
        pr_info("tx_isp_core_start: Invalid subdev\n");
        return -EINVAL;
    }

    isp_dev = sd->isp;
    if (!isp_dev) {
        pr_info("tx_isp_core_start: No ISP device\n");
        return -EINVAL;
    }
    ...
    if (!isp_dev->core_regs) {
        ...
        if (ret < 0) {
            pr_info("tx_isp_core_start: Failed to initialize memory mappings: %d\n", ret);
            return ret;
        }
    }
...
//        if (ret < 0) {
//            pr_info("tx_isp_core_start: Failed to configure clocks: %d\n", ret);
//            return ret;
//        }
//    }
    ...
    if (isp_dev->pipeline_state == ISP_PIPELINE_IDLE) {
        ...
        if (ret < 0) {
            pr_info("tx_isp_core_start: Failed to setup pipeline: %d\n", ret);
            return ret;
        }
    }
    ...
    if (isp_dev->core_regs) {
        ...
        pr_info("tx_isp_core_start: core_regs not mapped; skipping pipeline enable here\n");
    }
...
int tx_isp_core_stop(struct tx_isp_subdev *sd)
{
    ...
    if (!sd) {
        pr_info("tx_isp_core_stop: Invalid subdev\n");
        return -EINVAL;
    }

    isp_dev = sd->isp;
    if (!isp_dev) {
        pr_info("tx_isp_core_stop: No ISP device\n");
        return -EINVAL;
    }
...
int tx_isp_core_set_format(struct tx_isp_subdev *sd, struct tx_isp_config *config)
{
    ...
    if (!sd || !config) {
        pr_info("tx_isp_core_set_format: Invalid parameters\n");
        return -EINVAL;
    }

    isp_dev = sd->isp;
    if (!isp_dev) {
        pr_info("tx_isp_core_set_format: No ISP device\n");
        return -EINVAL;
    }
    ...
    if (ret < 0) {
        pr_info("tx_isp_core_set_format: Failed to configure format propagation: %d\n", ret);
        return ret;
    }
    ...
    if (isp_dev->sensor) {
        ...
        pr_info("tx_isp_core_set_format: No sensor present to determine CFA\n");
    }
...
static int ispcore_sensor_ops_ioctl(struct tx_isp_dev *isp_dev)
{
    ...
    if (!per_frame_sensor_ops_enabled) {
        pr_info("ispcore_sensor_ops_ioctl: per-frame ops disabled; skipping I2C\n");
        return 0;
    }
    ...
    if (isp_dev->sensor && isp_dev->sensor->sd.ops &&
        ...
        if (result == 0) {
            ...
            pr_info("*** ispcore_sensor_ops_ioctl: Sensor I2C communication failed: %d ***\n", result);
        }
    } else {
        pr_info("*** ispcore_sensor_ops_ioctl: No real sensor device found ***\n");
        result = -ENODEV;
    }
...
static void ispcore_irq_fs_work(struct work_struct *work)
{
    ...
    if (!isp_dev) {
        pr_info("*** ISP FRAME SYNC WORK: isp_dev is NULL ***\n");
        return;
    }
    ...
    if (isp_dev->streaming_enabled && vic_is_streaming && !csi_streaming_started) {
        ...
        if (isp_dev->csi_dev) {
            ...
            if (csi_result == 0) {
                ...
                pr_info("*** CSI STREAMING FAILED: %d - Error interrupt type 2 will continue ***\n", csi_result);
            }
        } else {
            pr_info("*** CSI STREAMING FAILED: No CSI device available ***\n");
        }
        ...
        if (isp_dev->csi_dev) {
            ...
            if (csi_result == 0) {
                ...
                pr_info("*** CSI STREAMING STOP FAILED: %d ***\n", csi_result);
            }
        } else {
            pr_info("*** CSI STREAMING STOP FAILED: No CSI device available ***\n");
        }
    }
    ...
    if (isp_dev->sensor && isp_dev->streaming_enabled) {
        ...
        if (per_frame_sensor_ops_enabled) {
            ...
            pr_info("Frame sync: per-frame sensor ops executed, result=%d\n", sensor_result);
        } else {
            pr_info("Frame sync: skipping per-frame sensor I2C ops (disabled)\n");
        }
        pr_info("Frame sync event processed (sensor available)\n");
    } else {
        pr_info("Frame sync event processed (no sensor/not streaming)\n");
    }
...
{
    if (index < 0 || index >= 32) {
        pr_info("system_irq_func_set: Invalid index %d\n", index);
        return -EINVAL;
    }
...
irqreturn_t tx_isp_core_irq_thread_handle(int irq, void *dev_id)
{
    ...
    pr_info("*** isp_irq_thread_handle: Thread IRQ %d, dev_id=%p ***\n", irq, dev_id);
...
int tx_isp_init_memory_mappings(struct tx_isp_dev *isp)
{
    ...
    if (!isp->core_regs) {
        pr_info("Failed to map ISP core registers\n");
        return -ENOMEM;
    }
    ...
    if (!isp->phy_base) {
        pr_info("Failed to map PHY registers\n");
        goto err_unmap_csi;
    }
...
int tx_isp_configure_clocks(struct tx_isp_dev *isp)
{
    ...
    if (IS_ERR(cgu_isp)) {
        pr_info("Failed to get CGU ISP clock\n");
    ...
    if (IS_ERR(isp_clk)) {
        pr_info("Failed to get ISP clock\n");
        ret = PTR_ERR(isp_clk);
    ...
    if (IS_ERR(ipu_clk)) {
        pr_info("Failed to get IPU clock\n");
        ret = PTR_ERR(ipu_clk);
    ...
    if (IS_ERR(csi_clk)) {
        pr_info("Failed to get CSI clock\n");
        ret = PTR_ERR(csi_clk);
...
int tx_isp_setup_pipeline(struct tx_isp_dev *isp)
{
    ...
    if (ret < 0) {
        pr_info("Failed to setup media links: %d\n", ret);
        return ret;
    }
    ...
    if (ret < 0) {
        pr_info("Failed to configure default links: %d\n", ret);
        return ret;
    }
...
static int tx_isp_setup_media_links(struct tx_isp_dev *isp)
{
    ...
    if (ret < 0) {
        pr_info("Failed to initialize subdev pads: %d\n", ret);
        return ret;
    }
    ...
    if (ret < 0) {
        pr_info("Failed to create subdev links: %d\n", ret);
        return ret;
    }
...
static int tx_isp_create_subdev_links(struct tx_isp_dev *isp)
{
    ...
    if (isp->csi_dev && isp->vic_dev) {
        ...
        if (ret < 0) {
            pr_info("Failed to register CSI->VIC link: %d\n", ret);
            return ret;
        }
...
{
    if (!isp || !link) {
        pr_info("Invalid parameters for link registration\n");
        return -EINVAL;
    }
...
int isp_malloc_buffer(struct tx_isp_dev *isp, uint32_t size, void **virt_addr, dma_addr_t *phys_addr)
{
    ...
    if (!isp || !virt_addr || !phys_addr || size == 0) {
        pr_info("isp_malloc_buffer: Invalid parameters\n");
        return -EINVAL;
    }
    ...
    virt = vmalloc(size);
    if (!virt) {
        pr_info("*** isp_malloc_buffer: Failed to allocate %u bytes from kernel memory ***\n", size);
        return -ENOMEM;
    }
...
int tisp_channel_stop(uint32_t channel_id)
{
    ...
    if (!isp_dev || channel_id >= ISP_MAX_CHAN) {
        pr_info("tisp_channel_stop: Invalid parameters\n");
        return -EINVAL;
    }
...
static int tx_isp_create_framechan_devices(struct tx_isp_dev *isp_dev)
{
    ...
    for (i = 0; i < 4; i++) {  /* Create 4 frame channels like reference */
        ...
        if (!fs_miscdev) {
            pr_info("Failed to allocate misc device for framechan%d\n", i);
            return -ENOMEM;
        }
        ...
        if (ret < 0) {
            pr_info("Failed to register /dev/%s: %d\n", dev_name, ret);
...
int tx_isp_core_probe(struct platform_device *pdev)
{
    ...
    if (!isp_dev->subdev_list) {
        pr_info("Failed to allocate subdev_list\n");
        kfree(isp_dev);
        return -ENOMEM;
    }
    ...
    if (tx_isp_subdev_init(pdev, &isp_dev->sd, &core_subdev_ops_full) == 0) {
        ...
        if (channel_array != NULL) {
            ...
            if (result != 0) {
                pr_info("*** tx_isp_core_probe: Failed to create VIC device: %d ***\n", result);
                return result;
            } else {
            ...
            result = tx_isp_init_memory_mappings(isp_dev);
                ...
                if (result != 0) {
                    pr_info("*** tx_isp_core_probe: Failed to create VIN device: %d ***\n", result);
                    return result;
                } else {
                ...
                if (!fs_workqueue) {
                    pr_info("*** tx_isp_core_probe: Failed to create frame sync workqueue ***\n");
                    return -ENOMEM;
                }
                ...
                if (result == 0) {
                    ...
                    pr_info("*** tx_isp_core_probe: tx_isp_create_graph_and_nodes FAILED: %d ***\n", result);
                }
                ...
                if (result == 0) {
                    ...
                    pr_info("*** tx_isp_core_probe: Failed to create frame channel devices: %d ***\n", result);
                }
                ...
                if (result == 0) {
                    ...
                    pr_info("*** tx_isp_core_probe: Failed to create ISP proc entries: %d ***\n", result);
                }
                ...
                if (result == 0) {
                    ...
                    pr_info("*** tx_isp_core_probe: Failed to create ISP M0 tuning device node: %d ***\n", result);
                }
...
int tiziano_sync_sensor_attr(struct tx_isp_sensor_attribute *attr)
{
    ...
    if (!attr) {
        pr_info("tiziano_sync_sensor_attr: Invalid sensor attributes\n");
        return -EINVAL;
    }
...
void private_dma_cache_sync(struct device *dev, void *vaddr, size_t size, enum dma_data_direction direction)
{
    ...
    if (!vaddr || size == 0) {
        pr_info("private_dma_cache_sync: Invalid parameters\n");
        return;
    }
