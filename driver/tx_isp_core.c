...
irqreturn_t ispcore_interrupt_service_routine(int irq, void *dev_id)
{
    ...
    if ((interrupt_status & 0x3f8) != 0) {
        ...
        pr_info("ISP CORE: Error status count: %d (Binary Ninja: data_ca57c += 1)\n", error_status_count);
    ...
    if (interrupt_status & 0x200) {
        ...
        pr_info("ISP CORE: Status bit 9 (0x200) - count: %d\n", status_200_count);
    ...
    if (interrupt_status & 0x100) {
        ...
        pr_info("ISP CORE: Status bit 8 (0x100) - count: %d\n", status_100_count);
...
/**
 * tisp_deinit - EXACT Binary Ninja MCP implementation
 * Address: 0x24ec0
 * Deinitializes the TISP (Tiziano ISP) subsystem
 */
int tisp_deinit(void)
{
    ...
    /* Binary Ninja: Free data_b2f3c through data_b2f9c (6 data pointers) */
    /* These are global data structures that need to be freed */

    /* Binary Ninja: int32_t $a0 = data_b2f3c */
    ...
    /* Binary Ninja: int32_t $a0_1 = data_b2f54 */
    ...
    /* Binary Ninja: int32_t $a0_2 = data_b2f6c */
    extern void *data_b2f6c;
    if (data_b2f6c != 0) {
        /* Binary Ninja: private_kfree($a0_2) */
        kfree(data_b2f6c);
        data_b2f6c = 0;
    }

    /* Binary Ninja: int32_t $a0_3 = data_b2f78 */
    extern void *data_b2f78;
    if (data_b2f78 != 0) {
        /* Binary Ninja: private_kfree($a0_3) */
        kfree(data_b2f78);
        data_b2f78 = 0;
    }

    /* Binary Ninja: int32_t $a0_4 = data_b2f84 */
    extern void *data_b2f84;
    if (data_b2f84 != 0) {
        /* Binary Ninja: private_kfree($a0_4) */
        kfree(data_b2f84);
        data_b2f84 = 0;
    }

    /* Binary Ninja: int32_t $a0_5 = data_b2f90 */
    extern void *data_b2f90;
    if (data_b2f90 != 0) {
        /* Binary Ninja: private_kfree($a0_5) */
        kfree(data_b2f90);
        data_b2f90 = 0;
    }

    /* Binary Ninja: int32_t $a0_6 = data_b2f9c */
    extern void *data_b2f9c;
    if (data_b2f9c != 0) {
        /* Binary Ninja: private_kfree($a0_6) */
        kfree(data_b2f9c);
        data_b2f9c = 0;
    }

    /* Binary Ninja: Free tparams_day, tparams_night, tparams_cust using vfree */
    /* uint32_t tparams_day_1 = tparams_day */
    extern void *tparams_day;
    if (tparams_day != 0) {
        /* Binary Ninja: private_vfree(tparams_day_1) */
        vfree(tparams_day);
        tparams_day = 0;
    }
    ...
    if (tparams_night != 0) {
        /* Binary Ninja: private_vfree(tparams_night_1) */
        vfree(tparams_night);
        tparams_night = 0;
    }
    ...
    if (tparams_cust != 0) {
        /* Binary Ninja: private_vfree(tparams_cust_1) */
        vfree(tparams_cust);
        tparams_cust = 0;
    }
