...
/* tx_isp_csi_slake_subdev - EXACT Binary Ninja implementation */
int tx_isp_csi_slake_subdev(struct tx_isp_subdev *sd)
{
    struct tx_isp_csi_device *csi_dev;
    struct clk **clks;
    ...
    if (sd != NULL) {
        ...
        if (csi_dev != NULL && (unsigned long)csi_dev < 0xfffff001) {
            ...
            /* Binary Ninja: if ($v1_2 == 4) */
            if (state == 4) {
                /* Binary Ninja: entry_$a2 = csi_video_s_stream(arg1, 0, entry_$a2) */
                csi_video_s_stream(sd, 0);
                state = csi_dev->state;
            }

            /* Binary Ninja: if ($v1_2 == 3) */
            if (state == 3) {
                /* Binary Ninja: csi_core_ops_init(arg1, 0, entry_$a2) */
                csi_core_ops_init(sd, 0);
            }
            ...
            /* Binary Ninja: if (*($s0_1 + 0x128) == 2) */
            if (csi_dev->state == 2) {
                /* Binary Ninja: *($s0_1 + 0x128) = 1 */
                csi_dev->state = 1;

                /* Binary Ninja: void* $v0 = *(arg1 + 0xbc) */
                clks = sd->clks;

                /* Binary Ninja: if ($v0 != 0 && $v0 u< 0xfffff001) */
                if (clks != NULL && (unsigned long)clks < 0xfffff001) {
                    /* Binary Ninja: int32_t $s0_2 = *(arg1 + 0xc0) */
                    clk_count = sd->clk_num;
            ...
            /* Binary Ninja: private_mutex_unlock($s2_1) */
            mutex_unlock(&csi_dev->mlock);
            /* Binary Ninja: return 0 */
            return 0;
        }
