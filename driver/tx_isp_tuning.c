...
int tisp_event_push(void *event)
{
    ...
    if (data_b33b8 == (uint32_t)&data_b33b8) {
        ...
        pr_debug("tisp_event_push: flags = 0x%08lx, jzflags = %p,0x%08x", flags, &data_b33b8, 0);
...
{
    if (!param_array) {
        pr_debug("tiziano_ae_set_hardware_param: NULL parameter array\n");
        return -EINVAL;
    }
    ...
    if (ae_id == 0) {
        ...
        pr_debug("tiziano_ae_set_hardware_param: Invalid AE ID %d\n", ae_id);
        return -EINVAL;
    }
...
int ae0_interrupt_static(void)
{
    ...
    if (data_b2f3c == 0) {
        pr_debug("*** CRITICAL: data_b2f3c is 0 - this would cause 22-second virt_to_phys() hang! ***\n");
        pr_debug("*** AE0 interrupt DISABLED to prevent system deadlock ***\n");
    ...
    if ((unsigned long)buffer_addr < 0x80000000 || (unsigned long)buffer_addr >= 0xfffff000) {
        pr_debug("*** CRITICAL: AE0 buffer_addr 0x%p outside kernel memory - would cause virt_to_phys() hang ***\n", buffer_addr);
        pr_debug("*** This would cause 22-second deadlock in DMA cache sync! ***\n");
        return 1;  /* Return success to prevent repeated calls */
    }

    /* CRITICAL SAFETY: Additional validation - check if address is actually mapped */
    if (!virt_addr_valid(buffer_addr)) {
        pr_debug("*** CRITICAL: AE0 buffer_addr 0x%p not valid virtual address ***\n", buffer_addr);
        pr_debug("*** AE0: Processing interrupt using register reads only (Binary Ninja MCP reference) ***\n");
        return 1;  /* Return success to prevent repeated calls */
    }
    ...
    pr_debug("*** AE0: Processing interrupt using register-based statistics (Binary Ninja MCP) ***\n");
...
int ae0_interrupt_hist(void)
{
    ...
    if (data_b2f3c == 0) {
        pr_debug("ae0_interrupt_hist: Data buffers not initialized - skipping safely\n");
        return 1;  /* Return success to prevent repeated calls */
    }
    ...
    pr_debug("ae0_interrupt_hist: AE0 histogram status = 0x%08x\n", ae0_hist_status);
...
int ae1_interrupt_static(void)
{
    ...
    /* CRITICAL SAFETY: Skip processing if data buffers not initialized */
    if (data_b2f3c == 0) {
        pr_debug("ae1_interrupt_static: Data buffers not initialized - skipping safely\n");
        return 1;  /* Return success to prevent repeated calls */
    }
    ...
    pr_debug("ae1_interrupt_static: AE1 status = 0x%08x\n", ae1_status);
...
int ae1_interrupt_hist(void)
{
    ...
    /* CRITICAL SAFETY: Skip processing if data buffers not initialized */
    if (data_b2f3c == 0) {
        pr_debug("ae1_interrupt_hist: Data buffers not initialized - skipping safely\n");
        return 1;  /* Return success to prevent repeated calls */
    }
    ...
    /* Binary Ninja: Process histogram data */
    pr_debug("ae1_interrupt_hist: AE1 histogram status = 0x%08x\n", ae1_hist_status);
...
int awb_interrupt_static(void)
{
    pr_debug("awb_interrupt_static: Processing AWB static interrupt\n");

    /* Binary Ninja: Read AWB status */
    uint32_t awb_status = system_reg_read(0xb004);

    /* Binary Ninja: Process AWB statistics */
    pr_debug("awb_interrupt_static: AWB status = 0x%08x\n", awb_status);
...
int tiziano_defog_interrupt_static(void)
{
    pr_debug("tiziano_defog_interrupt_static: Processing defog interrupt\n");

    /* Binary Ninja: Read defog status */
    uint32_t defog_status = system_reg_read(0x5004);

    /* Binary Ninja: Process defog statistics */
    pr_debug("tiziano_defog_interrupt_static: Defog status = 0x%08x\n", defog_status);
...
int tiziano_adr_interrupt_static(void)
{
    pr_debug("tiziano_adr_interrupt_static: Processing ADR interrupt\n");

    /* Binary Ninja: Read ADR status */
    uint32_t adr_status = system_reg_read(0x6004);

    /* Binary Ninja: Process ADR statistics */
    pr_debug("tiziano_adr_interrupt_static: ADR status = 0x%08x\n", adr_status);
...
int tiziano_wdr_interrupt_static(void)
{
    pr_debug("tiziano_wdr_interrupt_static: Processing WDR interrupt\n");

    /* Binary Ninja: Read WDR status */
    uint32_t wdr_status = system_reg_read(0x7004);

    /* Binary Ninja: Process WDR statistics */
    pr_debug("tiziano_wdr_interrupt_static: WDR status = 0x%08x\n", wdr_status);
...
static int load_isp_tuning_file(const char *filename)
{
    ...
    if (IS_ERR(fp)) {
        pr_debug("*** load_isp_tuning_file: Failed to open %s (error %ld) ***\n",
               filename, PTR_ERR(fp));
        return -ENOENT;
    }
    ...
    if (file_size <= 0) {
        pr_debug("*** load_isp_tuning_file: Invalid file size %d for %s ***\n",
               file_size, filename);
        filp_close(fp, NULL);
        return -EINVAL;
    }
    ...
    if (!file_buffer) {
        pr_debug("*** load_isp_tuning_file: Failed to allocate %d bytes ***\n", file_size);
        filp_close(fp, NULL);
        return -ENOMEM;
    }
    ...
    if (ret != file_size) {
        pr_debug("*** load_isp_tuning_file: Read %d bytes, expected %d ***\n", ret, file_size);
        vfree(file_buffer);
        return -EIO;
    }
    ...
    if (file_size >= 0x18 + 0x137f0 * 2) {  /* Header + day params + night params */
        ...
        if (!tparams_day) {
            ...
            if (!tparams_day) {
                pr_debug("*** load_isp_tuning_file: Failed to allocate day params ***\n");
                vfree(file_buffer);
                return -ENOMEM;
            }
        }
        ...
        if (!tparams_night) {
            ...
            if (!tparams_night) {
                pr_debug("*** load_isp_tuning_file: Failed to allocate night params ***\n");
                vfree(file_buffer);
                return -ENOMEM;
            }
        }
        ...
        if (!tparams_cust) {
            ...
            if (!tparams_cust) {
                pr_debug("*** load_isp_tuning_file: Failed to allocate custom params ***\n");
                vfree(file_buffer);
                return -ENOMEM;
            }
        }
        ...
        pr_debug("*** load_isp_tuning_file: Invalid file format or size ***\n");
        vfree(file_buffer);
        return -EINVAL;
    }
...
int tisp_init(void *sensor_info, char *param_name)
{
    ...
    if (!ourISPdev) {
        pr_debug("tisp_init: No ISP device available\n");
        return -ENODEV;
    }
    ...
    pr_debug("tisp_init: Using sensor parameters - %dx%d@%d, mode=%d\n",
    ...
    if (load_isp_tuning_file("/etc/sensor/gc2053-t31.bin") != 0) {
        pr_debug("*** tisp_init: Failed to load standard tuning file - using defaults ***\n");
    } else {
    ...
    if (load_isp_tuning_file("/etc/sensor/gc2053-cust-t31.bin") != 0) {
        pr_debug("*** tisp_init: Failed to load custom tuning file - using defaults ***\n");
    } else {
    ...
    pr_debug("tisp_init: Set ISP top bypass to 0x%x (reference-standardize)\n", bypass_val);
    ...
    pr_debug("*** tisp_init: STARTING EVENT PROCESSING THREAD ***\n");
    ...
    if (irq_ret == 0) {
        ...
        pr_debug("*** tisp_init: Failed to register ISP processing completion callback: %d ***\n", irq_ret);
    }

    /* Binary Ninja: tisp_param_operate_init() - Final parameter initialization */
    int param_init_ret = tisp_param_operate_init();
    if (param_init_ret != 0) {
        pr_debug("tisp_init: tisp_param_operate_init failed: %d\n", param_init_ret);
    ...
    if (ourISPdev->vic_dev) {
        ...
        if (pipo_ret == 0) {
            ...
            pr_debug("*** ERROR: tx_isp_subdev_pipo failed: %d ***\n", pipo_ret);
        }
    } else {
        pr_debug("*** ERROR: No VIC device available for tx_isp_subdev_pipo call ***\n");
    }
...
{
    if (!state) {
        pr_debug("Invalid AE state buffer\n");
        return -EINVAL;
    }
...
static int isp_get_ae_state(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    if (!ctrl) {
        pr_debug("No control structure for AE state\n");
        return -EINVAL;
    }
...
static int isp_get_af_zone(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    if (!ctrl) {
        pr_debug("No control structure for AF zone\n");
        return -EINVAL;
    }
...
static int apical_isp_core_ops_g_ctrl(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    if (!tuning) {
        pr_debug("apical_isp_core_ops_g_ctrl: No tuning data available\n");
        return -EINVAL;
    }
...
//                    return -EFAULT;
            ...
            default:
                pr_debug("Unknown m0 control get command: 0x%x\n", ctrl->cmd);
                ret = -EINVAL;
            break;
            }
        goto out;
    ...
    switch (ctrl->cmd) {
        ...
        case 0x980900:  // Brightness
            ...
            if (!access_ok(VERIFY_READ, &tuning->brightness, sizeof(tuning->brightness))) {
                pr_debug("CRITICAL: Cannot access brightness field - PREVENTS BadVA CRASH\n");
                return -EFAULT;
            }
        ...
        case 0x980901:  // Contrast
            ...
            if (!access_ok(VERIFY_READ, &tuning->contrast, sizeof(tuning->contrast))) {
                pr_debug("CRITICAL: Cannot access contrast field - PREVENTS BadVA CRASH\n");
                return -EFAULT;
            }
        ...
        case 0x980902:  // Saturation - CRITICAL FIX for BadVA crash
            ...
            if (!access_ok(VERIFY_READ, &tuning->saturation, sizeof(tuning->saturation))) {
                pr_debug("CRITICAL: Cannot access saturation field at %p - PREVENTING BadVA CRASH\n", &tuning->saturation);
                return -EFAULT;
            }
            ...
            if ((unsigned long)&tuning->saturation < (unsigned long)tuning ||
                ...
                pr_debug("CRITICAL: Saturation field address out of bounds - PREVENTING BadVA CRASH\n");
                return -EFAULT;
            }
...
static int apical_isp_core_ops_s_ctrl(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    if (!dev || !tuning) {
        pr_debug("No ISP device or tuning data\n");
        return -EINVAL;
    }
...
int isp_core_tunning_unlocked_ioctl(struct file *file, unsigned int cmd, void __user *arg)
{
    ...
    if (!ourISPdev) {
        pr_debug("*** TUNING IOCTL: ourISPdev is NULL - driver not fully initialized ***\n");
    ...
    if ((unsigned long)ourISPdev < 0x80000000 || (unsigned long)ourISPdev >= 0xfffff000) {
        pr_debug("*** TUNING IOCTL: Invalid ourISPdev pointer 0x%p - memory corruption ***\n", ourISPdev);
    ...
    dev = ourISPdev;
    if (!dev) {
        pr_debug("isp_core_tunning_unlocked_ioctl: No ISP device available\n");
        return -ENODEV;
    }
    ...
    if (magic == 0x56 && (!ourISPdev || ourISPdev->tuning_enabled != 3) && !auto_init_done) {
        ...
        if (!dev || !dev->tuning_data) {
            ...
            if (!dev || !dev->tuning_data) {
                pr_debug("isp_core_tunning_unlocked_ioctl: Failed to allocate tuning data\n");
                return -ENOMEM;
            }
    ...
    if (magic == 0x74 && (!ourISPdev || ourISPdev->tuning_enabled != 3)) {
        pr_debug("isp_core_tunning_unlocked_ioctl: Tuning commands require explicit enable (cmd=0x%x)\n", cmd);
        return -ENODEV;
    }
        ...
        switch (cmd) {
            case 0xc008561c: /* ISP_CORE_S_CTRL - Set control */
                ...
                if (copy_from_user(&ctrl, (void __user *)arg, sizeof(ctrl))) {
                    pr_debug("isp_core_tunning_unlocked_ioctl: Failed to copy control data from user\n");
                    return -EFAULT;
                }
                ...
                if (ctrl.cmd == 0x980900 && (!dev || !dev->tuning_data)) {
                    pr_debug("isp_core_tunning_unlocked_ioctl: Brightness control attempted with NULL tuning data\n");
                    return -ENODEV;
                }
                ...
                if (dev && dev->tuning_data) {
                    ...
                    if ((unsigned long)tuning < 0x80000000 || (unsigned long)tuning >= 0xfffff000) {
                        pr_debug("*** CRITICAL: tuning_data pointer 0x%p outside kernel memory - ABORTING ***\n", tuning);
                        return -EFAULT;
                    }

                    /* Validate alignment */
                    if ((unsigned long)tuning & 0x3) {
                        pr_debug("*** CRITICAL: tuning_data pointer 0x%p not 4-byte aligned - ABORTING ***\n", tuning);
                        return -EFAULT;
                    }
                }
                ...
                if (ret == 0 && copy_to_user((void __user *)arg, &ctrl, sizeof(ctrl))) {
                    pr_debug("isp_core_tunning_unlocked_ioctl: Failed to copy control data to user\n");
                    return -EFAULT;
                }
                break;
            ...
            case 0xc00c56c6: /* ISP_TUNING_ENABLE - Enable/disable tuning */
            {
                ...
                if (copy_from_user(&enable, (void __user *)arg, sizeof(enable))) {
                    pr_debug("isp_core_tunning_unlocked_ioctl: Failed to copy tuning enable data from user\n");
                    return -EFAULT;
                }
                ...
                if (enable && ourISPdev && ourISPdev->tuning_enabled == 3) {
                    ...
                    if (!tisp_par_ioctl) {
                        ...
                        if (tisp_par_ioctl) {
                            ...
                            pr_debug("*** CRITICAL: Failed to allocate tuning parameter buffer ***\n");
                        }
                    }
                    ...
                    if (ourISPdev->vic_dev) {
                        ...
                        pr_debug("*** CRITICAL FIX: Skipping isp_frame_done_wakeup() to prevent CSI PHY corruption ***\n");

                        /* Update frame counter for userspace - this is safe */
                        extern atomic64_t frame_done_cnt;
                        atomic64_inc(&frame_done_cnt);
                        pr_debug("*** Frame counter updated without triggering CSI PHY corruption ***\n");
                    }
            ...
            default:
                pr_debug("isp_core_tunning_unlocked_ioctl: Unknown ISP core control command: 0x%x\n", cmd);
                ret = -EINVAL;
                break;
        }
    ...
    if (magic == 0x74) {
        ...
        if (!tisp_par_ioctl) {
            pr_debug("tisp_code_tuning_ioctl: Global buffer not allocated\n");
            return -ENOMEM;
        }
                ...
                switch (cmd) {
                    case 0x20007400: { /* GET operation */
                        ...
                        if (!access_ok(VERIFY_READ, arg, 0x500c)) {
                            pr_debug("tisp_code_tuning_ioctl: Access check failed for GET\n");
                            return -EFAULT;
                        }

                        /* SECURITY FIX: Validate user buffer is not in kernel space */
                        if ((unsigned long)arg >= 0x80000000) {
                            pr_debug("tisp_code_tuning_ioctl: Invalid user buffer in kernel space\n");
                            return -EFAULT;
                        }
                        ...
                        if (!tisp_par_ioctl) {
                            ...
                            pr_debug("tisp_code_tuning_ioctl: Buffer was freed during operation\n");
                            return -ENODEV;
                        }
                        ...
                        if (ret != 0) {
                            pr_debug("tisp_code_tuning_ioctl: Copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        ...
                        if (param_type >= 0x19) {
                            ...
                            pr_debug("tisp_code_tuning_ioctl: Invalid GET parameter type %d\n", param_type);
                            return -EINVAL;
                        }
                        ...
                        ret = copy_to_user((void __user *)s0_1, tisp_par_ioctl, 0x500c);
                        if (ret != 0) {
                            pr_debug("tisp_code_tuning_ioctl: Final copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        break;
                    }
                    ...
                    case 0x20007401: { /* SET operation */
                        ...
                        if (!access_ok(VERIFY_WRITE, arg, 0x500c)) {
                            pr_debug("tisp_code_tuning_ioctl: Access check failed for SET\n");
                            return -EFAULT;
                        }

                        /* SECURITY FIX: Validate user buffer is not in kernel space */
                        if ((unsigned long)arg >= 0x80000000) {
                            pr_debug("tisp_code_tuning_ioctl: Invalid user buffer in kernel space for SET\n");
                            return -EFAULT;
                        }
                        ...
                        if (!tisp_par_ioctl) {
                            ...
                            pr_debug("tisp_code_tuning_ioctl: Buffer was freed during SET operation\n");
                            return -ENODEV;
                        }
                        ...
                        if (ret != 0) {
                            pr_debug("tisp_code_tuning_ioctl: Copy from user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        ...
                        if (param_type - 1 >= 0x18) {
                            pr_debug("tisp_code_tuning_ioctl: Invalid SET parameter type %d\n", param_type);
                            return -EINVAL;
                        }
                    ...
                    case 0x20007403: { /* AE info get */
                        ...
                        if (!access_ok(VERIFY_WRITE, arg, 0x500c)) {
                            pr_debug("tisp_code_tuning_ioctl: Access check failed for AE info get\n");
                            return -EFAULT;
                        }

                        ret = copy_to_user((void __user *)s0_1, tisp_par_ioctl, 0x500c);
                        if (ret != 0) {
                            pr_debug("tisp_code_tuning_ioctl: AE info copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        break;
                    }
                    ...
                    case 0x20007404: { /* AE info set */
                        ...
                        if (!access_ok(VERIFY_READ, arg, 0x500c)) {
                            pr_debug("tisp_code_tuning_ioctl: Access check failed for AE info set\n");
                            return -EFAULT;
                        }

                        ret = copy_from_user(tisp_par_ioctl, (void __user *)s0_1, 0x500c);
                        if (ret != 0) {
                            pr_debug("tisp_code_tuning_ioctl: AE info copy from user failed: %d\n", ret);
                            return -EFAULT;
                        }
                    ...
                    case 0x20007406: { /* AWB info get */
                        ...
                        if (!access_ok(VERIFY_WRITE, arg, 0x500c)) {
                            pr_debug("tisp_code_tuning_ioctl: Access check failed for AWB info get\n");
                            return -EFAULT;
                        }

                        ret = copy_to_user((void __user *)s0_1, tisp_par_ioctl, 0x500c);
                        if (ret != 0) {
                            pr_debug("tisp_code_tuning_ioctl: AWB info copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        break;
                    }
                    ...
                    case 0x20007407: { /* AWB info set */
                        ...
                        if (!access_ok(VERIFY_READ, arg, 0x500c)) {
                            pr_debug("tisp_code_tuning_ioctl: Access check failed for AWB info set\n");
                            return -EFAULT;
                        }

                        ret = copy_from_user(tisp_par_ioctl, (void __user *)s0_1, 0x500c);
                        if (ret != 0) {
                            pr_debug("tisp_code_tuning_ioctl: AWB info copy from user failed: %d\n", ret);
                            return -EFAULT;
                        }
                    ...
                    case 0x20007408: { /* Special operation 1 */
                        ...
                        if (!access_ok(VERIFY_READ, arg, 0x500c)) {
                            pr_debug("tisp_code_tuning_ioctl: Access check failed for special op 1\n");
                            return -EFAULT;
                        }

                        ret = copy_from_user(tisp_par_ioctl, (void __user *)s0_1, 0x500c);
                        if (ret != 0) {
                            pr_debug("tisp_code_tuning_ioctl: Special op 1 copy from user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        ...
                        ret = copy_to_user((void __user *)s0_1, tisp_par_ioctl, 0x500c);
                        if (ret != 0) {
                            pr_debug("tisp_code_tuning_ioctl: Special op 1 copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        break;
                    }
                    ...
                    case 0x20007409: { /* Special operation 2 */
                        ...
                        if (!access_ok(VERIFY_READ, arg, 0x500c)) {
                            pr_debug("tisp_code_tuning_ioctl: Access check failed for special op 2\n");
                            return -EFAULT;
                        }

                        ret = copy_from_user(tisp_par_ioctl, (void __user *)s0_1, 0x500c);
                        if (ret != 0) {
                            pr_debug("tisp_code_tuning_ioctl: Special op 2 copy from user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        ...
                        ret = copy_to_user((void __user *)s0_1, tisp_par_ioctl, 0x500c);
                        if (ret != 0) {
                            pr_debug("tisp_code_tuning_ioctl: Special op 2 copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        break;
                    }
                    ...
                    default:
                        pr_debug("tisp_code_tuning_ioctl: Unknown command in valid range: 0x%x\n", cmd);
                        return -EINVAL;
                }
    ...
    if (((cmd >> 8) & 0xff) != 0x74) {
        pr_debug("tisp_code_tuning_ioctl: Command magic not 0x74: cmd=0x%x\n", cmd);
        return -EINVAL;
    } else {
        pr_debug("tisp_code_tuning_ioctl: Command out of valid range: cmd=0x%x\n", cmd);
        return -EINVAL;
    }
}
...
int tisp_code_tuning_open(struct inode *inode, struct file *file)
{
    ...
    if (!tuning_buffer) {
        pr_debug("tisp_code_tuning_open: Failed to allocate tuning buffer (0x%x bytes)\n", TUNING_PARAM_BUFFER_SIZE);
        return -ENOMEM;
    }

    /* CRITICAL: Verify alignment for MIPS - must be 4-byte aligned */
    if ((unsigned long)tuning_buffer & 0x3) {
        pr_debug("CRITICAL: Tuning buffer not 4-byte aligned: %p\n", tuning_buffer);
        kfree(tuning_buffer);
        return -ENOMEM;
    }
...
long tisp_code_tuning_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    if (((cmd >> 8) & 0xFF) == 0x74) {
            ...
            if ((cmd - 0x20007400) < 0xa) {
                ...
                switch (cmd) {
                    case 0x20007400: /* Get parameter configuration */
                    {
                        ...
                        if (!tisp_par_ioctl) {
                            pr_debug("tisp_code_tuning_ioctl: Parameter buffer not allocated\n");
                            return -ENOMEM;
                        }

                        if (copy_from_user(tisp_par_ioctl, argp, 0x500c)) {
                            pr_debug("tisp_code_tuning_ioctl: Failed to copy parameters from user\n");
                            return -EFAULT;
                        }
                        ...
                        switch (param_type) {
                            ...
                            case 0x18: /* tisp_dn_mode_get */
                                ...
                                break;
                            default:
                                pr_debug("tisp_code_tuning_ioctl: Unknown get parameter type %d\n", param_type);
                                ret = -EINVAL;
                                break;
                        }
                        ...
                        if (ret == 0) {
                            if (copy_to_user(argp, tisp_par_ioctl, 0x500c)) {
                                pr_debug("tisp_code_tuning_ioctl: Failed to copy parameters to user\n");
                                return -EFAULT;
                            }
                        }
                    ...
                    case 0x20007401: /* Set parameter configuration */
                    {
                        ...
                        if ((param_type - 1) >= 0x18) {
                            pr_debug("tisp_code_tuning_ioctl: Invalid parameter type %d\n", param_type);
                            return -EINVAL;
                        }
                        ...
                        switch (param_type) {
                            ...
                            case 0x15: /* tisp_awb_set_par_cfg */
                                ...
                                break;
                            case 0x16: /* Reserved */
                                pr_debug("tisp_code_tuning_ioctl: Reserved parameter type 0x16\n");
                                ret = -EINVAL;
                                break;
                    ...
                    default:
                        pr_debug("tisp_code_tuning_ioctl: Unknown tuning command 0x%x\n", cmd);
                        return -EINVAL;
                }
        ...
        pr_debug("tisp_code_tuning_ioctl: Command out of range: 0x%x\n", cmd);
        return -EINVAL;
    }

    /* Binary Ninja: Handle non-0x74 commands */
    pr_debug("tisp_code_tuning_ioctl: Invalid command family: 0x%x\n", cmd);
    return -EINVAL;
}
...
int tisp_code_tuning_release(struct inode *inode, struct file *file)
{
    ...
    if (tisp_par_ioctl) {
        ...
        pr_debug("tisp_code_tuning_release: Parameter buffer marked for cleanup\n");
    }
    ...
    if (buffer_to_free) {
        ...
        pr_debug("tisp_code_tuning_release: Parameter buffer freed safely\n");
    }
...
int apical_isp_ae_g_roi(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    if (buffer == NULL) {
        ...
        pr_debug("apical_isp_ae_g_roi: not support the gpio mode!\n");
    ...
    if (result == 0) {
        ...
        pr_debug("apical_isp_ae_g_roi: width/height/imagesize error\n");
    }
...
{
    if (!out_buf) {
        pr_debug("tisp_g_wdr_en: NULL output buffer\n");
        return -EINVAL;
    }
...
int tisp_gb_param_array_get(int param_id, void *out_buf, int *size_buf)
{
    ...
    if ((param_id - 0x3f5) >= 0xa) {
        pr_debug("tisp_gb_param_array_get: Invalid parameter ID 0x%x\n", param_id);
        return -1;
    }

    if (!out_buf || !size_buf) {
        pr_debug("tisp_gb_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    switch (param_id) {
        ...
        case 0x3fe:  /* tisp_gb_blc_min */
            ...
            break;
        default:
            pr_debug("tisp_gb_param_array_get: Unhandled parameter ID 0x%x\n", param_id);
            return -1;
    }
...
int tisp_lsc_param_array_get(int param_id, void *out_buf, int *size_buf)
{
    ...
    if ((param_id - 0x54) >= 0xb) {
        pr_debug("tisp_lsc_param_array_get: Invalid parameter ID 0x%x\n", param_id);
        return -1;
    }

    if (!out_buf || !size_buf) {
        pr_debug("tisp_lsc_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    switch (param_id) {
        ...
        case 0x5e:  /* lsc_mean_en */
            ...
            break;
        default:
            pr_debug("tisp_lsc_param_array_get: Unhandled parameter ID 0x%x\n", param_id);
            return -1;
    }
...
int tisp_wdr_param_array_get(int param_id, void *out_buf, int *size_buf)
{
    ...
    if ((param_id - 0x3ff) >= 0x33) {
        pr_debug("tisp_wdr_param_array_get: Invalid parameter ID 0x%x\n", param_id);
        return -1;
    }

    if (!out_buf || !size_buf) {
        pr_debug("tisp_wdr_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
int tisp_wdr_param_array_get_extended(int param_id, void *out_buf, int *size_buf)
{
    ...
    switch (param_id) {
        ...
        case 0x431:  /* param_wdr_tool_control_array */
            ...
            break;
        default:
            pr_debug("tisp_wdr_param_array_get_extended: Unhandled parameter ID 0x%x\n", param_id);
            return -1;
    }
...
int tisp_dpc_param_array_get(int param_id, void *out_buf, int *size_buf)
{
    ...
    if ((param_id - 0xe6) >= 0x1f) {
        pr_debug("tisp_dpc_param_array_get: Invalid parameter ID 0x%x\n", param_id);
        return -1;
    }

    if (!out_buf || !size_buf) {
        pr_debug("tisp_dpc_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    switch (param_id) {
        ...
        case 0x104:  /* ctr_con_par_array */
            ...
            break;
        default:
            pr_debug("tisp_dpc_param_array_get: Unhandled parameter ID 0x%x\n", param_id);
            return -1;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_rdns_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_adr_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_ccm_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_gamma_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_defog_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_top_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_blc_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_lsc_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_wdr_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_dpc_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_mdns_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_ydns_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_bcsh_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_clm_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_ysp_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_sdns_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_af_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
int tisp_g_aeroi_weight(void *buffer)
{
    ...
    pr_debug("tisp_g_aeroi_weight: bank no free\n");
...
int tisp_g_aezone_weight(void *buffer)
{
    ...
    pr_debug("tisp_g_aezone_weight: Failed to allocate vic device\n");
    return -1;  /* Binary Ninja returns 0xffffffff */
}
...
int tisp_ae_param_array_get(int param_type, void *buffer, int *size)
{
    ...
    if ((param_type - 1) >= 0x22) {
        pr_debug("tisp_ae_param_array_get: Invalid parameter type %d\n", param_type);
        return -1;  /* Binary Ninja returns 0xffffffff */
    }
    ...
    switch (param_type) {
        ...
        case 0x22: /* ae1_comp_ev_list */
            ...
            break;
        default:
            pr_debug("tisp_ae_param_array_get: Unhandled parameter type %d\n", param_type);
            return -1;
    }
...
static int tiziano_wdr_algorithm(void)
{
    ...
    if (!data_d94f8) {
        ...
        if (!data_d94f8) {
            pr_debug("tiziano_wdr_algorithm: Failed to allocate output array\n");
            return -ENOMEM;
        }
...
int tiziano_ae_init(uint32_t height, uint32_t width, uint32_t fps)
{
    ...
    *data_d04c4 = height;
    ...
    pr_debug("*** DEBUGGING: Registering ONLY callback for bit 10 (status 0x400) ***\n");
    ...
    pr_debug("*** CRITICAL FIX: Skipping NULL spinlock initialization that was causing 6+ second delays ***\n");
...
int tiziano_gamma_lut_parameter(void)
{
    ...
    if (!base_reg) {
        pr_debug("tiziano_gamma_lut_parameter: Failed to map gamma registers\n");
        return -ENOMEM;
    }

    if (!tiziano_gamma_lut_now) {
        pr_debug("tiziano_gamma_lut_parameter: No gamma LUT selected\n");
        iounmap(base_reg);
        return -EINVAL;
    }
...
static int tiziano_ccm_lut_parameter(int32_t *ccm_data)
{
    ...
    if (!base_reg) {
        pr_debug("tiziano_ccm_lut_parameter: Failed to map CCM registers\n");
        return -ENOMEM;
    }
...
int tiziano_ccm_init(void)
{
    ...
    int ret = jz_isp_ccm();
    if (ret) {
        pr_debug("tiziano_ccm_init: Failed to initialize CCM: %d\n", ret);
        return ret;
    }
...
int tiziano_dmsc_init(void)
{
    pr_debug("tiziano_dmsc_init: Initializing DMSC processing (BN MCP)\n");
...
void tisp_dmsc_set_cfa_from_mbus(uint32_t mbus_code, int hflip, int vflip)
{
    ...
    pr_debug("tisp_dmsc_set_cfa_from_mbus: FORCED GC2053 mosaic=%u (mbus=0x%x h=%d v=%d) out_opt=0x%08x\n",
...
static int tisp_sharpen_all_reg_refresh(void)
{
    ...
    if (!base_reg) {
        pr_debug("tisp_sharpen_all_reg_refresh: Failed to map sharpening registers\n");
        return -ENOMEM;
    }
...
int tiziano_sharpen_init(void)
{
    ...
    if (ret) {
        pr_debug("tiziano_sharpen_init: Failed to refresh sharpening parameters: %d\n", ret);
        return ret;
    }
...
static int tisp_sdns_all_reg_refresh(void)
{
    ...
    if (!base_reg) {
        pr_debug("tisp_sdns_all_reg_refresh: Failed to map SDNS registers\n");
        return -ENOMEM;
    }
...
int tiziano_sdns_init(void)
{
    ...
    if (ret) {
        pr_debug("tiziano_sdns_init: Failed to refresh SDNS parameters: %d\n", ret);
        return ret;
    }
...
int tiziano_mdns_init(uint32_t width, uint32_t height)
{
    ...
    if (!base_reg) {
        pr_debug("tiziano_mdns_init: Failed to map MDNS registers\n");
        return -ENOMEM;
    }
...
static int tisp_dpc_all_reg_refresh(void)
{
    ...
    if (!base_reg) {
        pr_debug("tisp_dpc_all_reg_refresh: Failed to map DPC registers\n");
        return -ENOMEM;
    }
...
static int tisp_adr_set_params(void)
{
    ...
    if (!base_reg) {
        pr_debug("tisp_adr_set_params: Failed to map ADR registers\n");
        return -ENOMEM;
    }
...
int tiziano_adr_init(uint32_t width, uint32_t height)
{
    ...
    if (ret) {
        pr_debug("tiziano_adr_init: Failed to set ADR parameters: %d\n", ret);
        return ret;
    }
...
int tisp_event_set_cb(int event_id, void *callback)
{
    ...
    if (event_id < 0 || event_id >= 32) {
        pr_debug("tisp_event_set_cb: Invalid event ID %d\n", event_id);
        return -EINVAL;
    }
...
static irqreturn_t isp_irq_dispatcher(int irq, void *dev_id)
{
    ...
    if (!dev || !dev || !dev->core_regs) {
        pr_debug("isp_irq_dispatcher: Invalid device or register base\n");
        return IRQ_NONE;
    }
...
static int isp_event_dispatcher(int event_id)
{
    ...
    if (event_id < 0 || event_id >= 32) {
        pr_debug("isp_event_dispatcher: Invalid event ID %d\n", event_id);
        return -EINVAL;
    }
...
int tisp_event_init(void)
{
    ...
    pr_debug("tisp_event_init: SAFE event system initialized with %d nodes\n", EVENT_NODE_COUNT);
...
int tisp_event_process(void)
{
    ...
    if (ret == -ERESTARTSYS) {
        ...
        pr_debug("tisp_event_process: Event processing interrupted by signal\n");
        return 0;
    }
    ...
    if (ourISPdev && ourISPdev && ourISPdev->tuning_data) {
        ...
        if (sensor && sensor->sd.ops &&
            ...
            if (sensor_ret == 0) {
                ...
                pr_debug("tisp_again_update: Sensor I2C gain update FAILED: %d\n", sensor_ret);
            }
        } else {
            pr_debug("tisp_again_update: No sensor available for I2C communication\n");
        }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_ccm_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_gamma_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_sharpen_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_mdns_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_defog_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_rdns_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_clm_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_adr_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_ydns_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_af_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_sdns_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_bcsh_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
struct tx_isp_sensor *tx_isp_get_sensor(void)
{
    ...
    if (!ourISPdev) {
        pr_debug("*** tx_isp_get_sensor: ourISPdev is NULL ***\n");
        return NULL;
    }

    pr_debug("*** tx_isp_get_sensor: Searching subdev array for sensors ***\n");
    ...
    if (!real_sensor_found) {
        pr_debug("*** tx_isp_get_sensor: No real sensor modules found - returning NULL ***\n");
        return NULL;
    }
        ...
        if (candidate && candidate->ops && candidate->ops->sensor) {
            ...
            if (candidate->pdev && candidate->pdev->name) {
                if (strcmp(candidate->pdev->name, "isp-m0") != 0 &&
                    ...
                    sd = candidate;
                    pr_debug("*** tx_isp_get_sensor: Found real sensor subdev at index %d: %p (name=%s) ***\n",
                            i, sd, sd->pdev->name);
                    break;
                }
            }
    ...
    if (sd && sd->ops && sd->ops->sensor) {
        ...
        if (sd->pdev && sd->pdev->name && strcmp(sd->pdev->name, "isp-m0") != 0) {
            ...
            if (!sensor) {
                pr_debug("*** tx_isp_get_sensor: Found sensor subdev but no host_priv - creating sensor structure ***\n");
                /* Create a sensor structure for this subdev */
                sensor = kzalloc(sizeof(struct tx_isp_sensor), GFP_KERNEL);
                if (!sensor) {
                    pr_debug("*** tx_isp_get_sensor: Failed to allocate sensor structure ***\n");
                    return NULL;
                }
            ...
            pr_debug("*** tx_isp_get_sensor: Found real sensor: %p ***\n", sensor);

            /* CRITICAL FIX: If sensor attributes are NULL, set up default sensor attributes */
            if (!sensor->video.attr) {
                pr_debug("*** tx_isp_get_sensor: Sensor attributes are NULL - setting up default attributes ***\n");
                ...
                pr_debug("*** tx_isp_get_sensor: Default sensor attributes set up successfully ***\n");
            }

            return sensor;
        } else {
            pr_debug("*** tx_isp_get_sensor: Skipping Core device (not a real sensor) ***\n");
        }
    }

    pr_debug("*** tx_isp_get_sensor: No real sensor found in subdev array ***\n");
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_ae_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_hldc_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!out_buf || !size_buf) {
        pr_debug("tisp_awb_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
...
{
    if (!reg_val || !size_buf) {
        pr_debug("tisp_reg_map_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
int tisp_dn_mode_get(void *mode_buf, void *size_buf)
{
    if (!mode_buf || !size_buf) {
        pr_debug("tisp_dn_mode_get: NULL buffer pointers\n");
        return -EINVAL;
    }
...
int tisp_code_create_tuning_node(void)
{
    ...
    if (tuning_major == 0) {
        ...
        if (ret < 0) {
            pr_debug("tisp_code_create_tuning_node: Failed to allocate chrdev region: %d\n", ret);
            return ret;
        }
        ...
        if (ret < 0) {
            pr_debug("tisp_code_create_tuning_node: Failed to register chrdev region: %d\n", ret);
            return ret;
        }
    ...
    if (ret < 0) {
        pr_debug("tisp_code_create_tuning_node: Failed to add cdev: %d\n", ret);
    ...
    if (IS_ERR(tuning_class)) {
        ...
        pr_debug("tisp_code_create_tuning_node: Failed to create class: %d\n", ret);
    ...
    if (device_create(tuning_class, NULL, tuning_devno, NULL, "isp-m0") == NULL) {
        pr_debug("tisp_code_create_tuning_node: Failed to create device\n");
...
int tisp_s_Gamma(void *gamma_data)
{
    ...
    if (!gamma_data) {
        pr_debug("tisp_s_Gamma: NULL gamma data\n");
        return -EINVAL;
    }
...
int tisp_s_adr_enable(int enable)
{
    ...
    if (enable == 1) {
        ...
        pr_debug("tisp_s_adr_enable: Invalid enable value %d\n", enable);
        return -EINVAL;
    }
    ...
    pr_debug("*** CRITICAL ROOT CAUSE FIX: tisp_s_adr_enable - Skipping CSI PHY register 0xc write ***\n");
    pr_debug("*** Register 0xc is CSI PHY Control - ADR enable should not write to it! ***\n");
...
int tisp_s_ae_attr(void *ae_attr_data)
{
    ...
    if (!ae_attr_data) {
        pr_debug("tisp_s_ae_attr: NULL AE attribute data\n");
        return -EINVAL;
    }
...
int tisp_s_ae_hist(void *hist_data)
{
    ...
    if (!hist_data) {
        pr_debug("tisp_s_ae_hist: NULL histogram data\n");
        return -EINVAL;
    }
...
void *isp_core_tuning_init(void *arg1)
{
    ...
    pr_debug("isp_core_tuning_init: Initializing tuning data structure\n");
    ...
    if (!tuning_data) {
        pr_debug("isp_core_tuning_init: Failed to allocate tuning data structure\n");
        return NULL;
    }

    /* SAFE: Verify alignment for MIPS - must be 4-byte aligned */
    if ((unsigned long)tuning_data & 0x3) {
        pr_debug("CRITICAL: Tuning data not 4-byte aligned: %p\n", tuning_data);
        kfree(tuning_data);
        return NULL;
    }
    ...
    pr_debug("isp_core_tuning_init: Tuning data structure initialized at %p\n", tuning_data);
    pr_debug("isp_core_tuning_init: Structure size: %zu bytes (vs Binary Ninja 0x40d0)\n", sizeof(struct isp_tuning_data));
    pr_debug("*** SAFE: mode_flag properly initialized using struct member access ***\n");
...
int isp_core_tuning_event(void *arg1, int arg2)
{
    ...
    if (!tuning_data) {
        pr_debug("isp_core_tuning_event: NULL tuning data pointer\n");
        return -EINVAL;
    }
    ...
    if (arg2 == 0x4000001) {
        ...
        tuning_data->state = 1;
        pr_debug("isp_core_tuning_event: Set tuning state to active\n");
    } else {
        ...
        if ((unsigned int)arg2 >= 0x4000002) {
            ...
            if (arg2 == 0x4000002) {
                ...
                pr_debug("isp_core_tuning_event: Day/night control called with mode %u\n", day_night_mode);
            }
        ...
        if (arg2 == 0x4000000) {
            ...
            tuning_data->state = 2;
            pr_debug("isp_core_tuning_event: Set tuning state to 2\n");
        }
    }
...
{
    pr_debug("tisp_day_or_night_s_ctrl: Setting day/night mode to %u\n", mode);
...
void check_csi_error(void)
{
    ...
    if (!isp_dev || !isp_dev->csi_dev) {
        pr_debug("check_csi_error: No CSI device available\n");
        return;
    }
    ...
    while (1) {
        ...
        if (csi_regs) {
            ...
            if (csi_err1 != 0) {
                pr_debug("CSI Error 1 (0x20): 0x%x\n", csi_err1);
            }

            /* Binary Ninja: if ($s3_1 != 0) isp_printf(...) */
            if (csi_err2 != 0) {
                pr_debug("CSI Error 2 (0x24): 0x%x\n", csi_err2);
            }
        }
...
int tiziano_deflicker_expt(uint32_t flicker_t, uint32_t param2, uint32_t param3, uint32_t param4, uint32_t *lut_array, uint32_t *nodes_count)
{
    ...
    if (!lut_array || !nodes_count) {
        pr_debug("tiziano_deflicker_expt: NULL pointer parameters\n");
        return -EINVAL;
    }
...
void tisp_set_sensor_integration_time(uint32_t time)
{
    ...
    extern struct tx_isp_dev *ourISPdev;
    if (!ourISPdev) {
        pr_debug("tisp_set_sensor_integration_time: No ISP device available\n");
        return;
    }
...
int data_b2eec(uint32_t time, void **var_ptr)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_debug("data_b2eec: No sensor available\n");
...
int data_b2ef0(uint32_t time, void **var_ptr)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_debug("data_b2ef0: No sensor available\n");
        if (var_ptr) *var_ptr = NULL;
        return time;
    }
...
int data_b2ef4(uint32_t param, int flag)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_debug("data_b2ef4: No sensor available\n");
        return -ENODEV;
    }
...
int data_b2ef8(uint32_t param, int flag)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_debug("data_b2ef8: No sensor available\n");
        return -ENODEV;
    }
...
uint32_t data_b2ee0(uint32_t log_val, int16_t *var_ptr)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_debug("data_b2ee0: No sensor available\n");
...
uint32_t data_b2ee4(uint32_t log_val, void **var_ptr)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_debug("data_b2ee4: No sensor available\n");
        if (var_ptr) *var_ptr = NULL;
        return log_val;
    }
...
int data_b2f04(uint32_t param, int flag)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_debug("data_b2f04: No sensor available\n");
        return -ENODEV;
    }
...
int data_b2f08(uint32_t param, int flag)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_debug("data_b2f08: No sensor available\n");
        return -ENODEV;
    }
