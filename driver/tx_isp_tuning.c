...
int isp_core_tunning_unlocked_ioctl(struct file *file, unsigned int cmd, void __user *arg)
{
    ...
    if (magic == 0x56 && (!ourISPdev || ourISPdev->core_dev->tuning_enable != 3) && !auto_init_done) {
        ...
        ourISPdev->core_dev->tuning_enable = 3;
        auto_init_done = true;
    ...
    if (magic == 0x74 && (!ourISPdev || ourISPdev->core_dev->tuning_enable != 3)) {
    ...
    if (magic == 0x56) {
        ...
        switch (cmd) {
            ...
            case 0xc00c56c6: /* ISP_TUNING_ENABLE - Enable/disable tuning */
            {
                ...
                if (enable && ourISPdev && ourISPdev->core_dev->tuning_enable == 3) {
                    ...
                    break;
                            ...
                            int dpc_ret = tisp_dpc_par_refresh((dev && dev->tuning_data) ? ((struct isp_tuning_data *)ourISPdev->core_dev->tuning_data)->exposure >> 10 : 0x100, 0x20, 0);
                            ...
                            int sharpen_ret = tisp_sharpen_par_refresh((dev && dev->tuning_data) ? ((struct isp_tuning_data *)ourISPdev->core_dev->tuning_data)->exposure >> 10 : 0x100, 0x20, 0);
                            ...
                            int sdns_ret = tisp_sdns_par_refresh((dev && dev->tuning_data) ? ((struct isp_tuning_data *)ourISPdev->core_dev->tuning_data)->exposure >> 10 : 0x100, 0x20, 0);
...
int isp_m0_chardev_release(struct inode *inode, struct file *file)
{
    ...
    if (ourISPdev && ourISPdev && ourISPdev->core_dev->tuning_enable == 3) {
        pr_info("Disabling tuning on release\n");
        isp_core_tuning_release(ourISPdev);
        ourISPdev->core_dev->tuning_enable = 0;
    }
