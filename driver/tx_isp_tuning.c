...
int tisp_event_push(void *event)
{
    ...
    if (data_b33b8 == (uint32_t)&data_b33b8) {
        ...
        pr_info("tisp_event_push: flags = 0x%08lx, jzflags = %p,0x%08x", flags, &data_b33b8, 0);
...
{
    if (!param_array) {
        pr_info("tiziano_ae_set_hardware_param: NULL parameter array\n");
        return -EINVAL;
    }

    pr_info("tiziano_ae_set_hardware_param: ae_id=%d, update_only=%d\n", ae_id, update_only);
    ...
    if (ae_id == 0) {
        ...
        pr_info("tiziano_ae_set_hardware_param: Invalid AE ID %d\n", ae_id);
        return -EINVAL;
    }

    pr_info("tiziano_ae_set_hardware_param: Parameters written to AE%d\n", ae_id);
    return 0;
}
...
int ae0_interrupt_static(void)
{
    ...
    if (data_b2f3c == 0) {
        pr_info("*** CRITICAL: data_b2f3c is 0 - this would cause 22-second virt_to_phys() hang! ***\n");
        pr_info("*** AE0 interrupt DISABLED to prevent system deadlock ***\n");
    ...
    if ((unsigned long)buffer_addr < 0x80000000 || (unsigned long)buffer_addr >= 0xfffff000) {
        pr_info("*** CRITICAL: AE0 buffer_addr 0x%p outside kernel memory - would cause virt_to_phys() hang ***\n", buffer_addr);
        pr_info("*** This would cause 22-second deadlock in DMA cache sync! ***\n");
        return 1;  /* Return success to prevent repeated calls */
    }

    /* CRITICAL SAFETY: Additional validation - check if address is actually mapped */
    if (!virt_addr_valid(buffer_addr)) {
        pr_info("*** CRITICAL: AE0 buffer_addr 0x%p not valid virtual address ***\n", buffer_addr);
...
{
    pr_info("tisp_ae0_process: Starting AE0 processing\n");
    ...
    pr_info("tisp_ae0_process: AE0 processing completed\n");
    return 0;
}
...
static int load_isp_tuning_file(const char *filename)
{
    ...
    if (IS_ERR(fp)) {
        pr_info("*** load_isp_tuning_file: Failed to open %s (error %ld) ***\n",
               filename, PTR_ERR(fp));
        return -ENOENT;
    }
    ...
    if (file_size <= 0) {
        pr_info("*** load_isp_tuning_file: Invalid file size %d for %s ***\n",
               file_size, filename);
        filp_close(fp, NULL);
        return -EINVAL;
    }
    ...
    if (!file_buffer) {
        pr_info("*** load_isp_tuning_file: Failed to allocate %d bytes ***\n", file_size);
        filp_close(fp, NULL);
        return -ENOMEM;
    }
    ...
    if (ret != file_size) {
        pr_info("*** load_isp_tuning_file: Read %d bytes, expected %d ***\n", ret, file_size);
        vfree(file_buffer);
        return -EIO;
    }
    ...
    if (file_size >= 0x18 + 0x137f0 * 2) {  /* Header + day params + night params */
        ...
        if (!tparams_day) {
            ...
            if (!tparams_day) {
                pr_info("*** load_isp_tuning_file: Failed to allocate day params ***\n");
                vfree(file_buffer);
                return -ENOMEM;
            }
        }
        ...
        if (!tparams_night) {
            ...
            if (!tparams_night) {
                pr_info("*** load_isp_tuning_file: Failed to allocate night params ***\n");
                vfree(file_buffer);
                return -ENOMEM;
            }
        }
        ...
        if (!tparams_cust) {
            ...
            if (!tparams_cust) {
                pr_info("*** load_isp_tuning_file: Failed to allocate custom params ***\n");
                vfree(file_buffer);
                return -ENOMEM;
            }
        }
        ...
        pr_info("*** load_isp_tuning_file: Invalid file format or size ***\n");
        vfree(file_buffer);
        return -EINVAL;
    }
...
int tisp_init(void *sensor_info, char *param_name)
{
    ...
    if (!ourISPdev) {
        pr_info("tisp_init: No ISP device available\n");
        return -ENODEV;
    }
    ...
    if (load_isp_tuning_file("/etc/sensor/gc2053-t31.bin") != 0) {
        pr_info("*** tisp_init: Failed to load standard tuning file - using defaults ***\n");
    } else {
    ...
    if (load_isp_tuning_file("/etc/sensor/gc2053-cust-t31.bin") != 0) {
        pr_info("*** tisp_init: Failed to load custom tuning file - using defaults ***\n");
    } else {
    ...
    if (irq_ret == 0) {
        ...
        pr_info("*** tisp_init: Failed to register ISP processing completion callback: %d ***\n", irq_ret);
    }

    /* Binary Ninja: tisp_param_operate_init() - Final parameter initialization */
    int param_init_ret = tisp_param_operate_init();
    if (param_init_ret != 0) {
        pr_info("tisp_init: tisp_param_operate_init failed: %d\n", param_init_ret);
    ...
    if (ourISPdev->vic_dev) {
        ...
        if (pipo_ret == 0) {
            ...
            pr_info("*** ERROR: tx_isp_subdev_pipo failed: %d ***\n", pipo_ret);
        }
    } else {
        pr_info("*** ERROR: No VIC device available for tx_isp_subdev_pipo call ***\n");
    }
...
{
    if (!state) {
        pr_info("Invalid AE state buffer\n");
        return -EINVAL;
    }
...
static int isp_get_ae_state(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    if (!ctrl) {
        pr_info("No control structure for AE state\n");
        return -EINVAL;
    }
...
static int isp_get_af_zone(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    if (!ctrl) {
        pr_info("No control structure for AF zone\n");
        return -EINVAL;
    }
...
static int apical_isp_core_ops_g_ctrl(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    if (!tuning) {
        pr_info("apical_isp_core_ops_g_ctrl: No tuning data available\n");
        return -EINVAL;
    }
...
//                    return -EFAULT;
            ...
            default:
                pr_info("Unknown m0 control get command: 0x%x\n", ctrl->cmd);
                ret = -EINVAL;
            break;
            }
        goto out;
    ...
    switch (ctrl->cmd) {
        ...
        case 0x980900:  // Brightness
            ...
            if (!access_ok(VERIFY_READ, &tuning->brightness, sizeof(tuning->brightness))) {
                pr_info("CRITICAL: Cannot access brightness field - PREVENTS BadVA CRASH\n");
                return -EFAULT;
            }
        ...
        case 0x980901:  // Contrast
            ...
            if (!access_ok(VERIFY_READ, &tuning->contrast, sizeof(tuning->contrast))) {
                pr_info("CRITICAL: Cannot access contrast field - PREVENTS BadVA CRASH\n");
                return -EFAULT;
            }
        ...
        case 0x980902:  // Saturation - CRITICAL FIX for BadVA crash
            ...
            if (!access_ok(VERIFY_READ, &tuning->saturation, sizeof(tuning->saturation))) {
                pr_info("CRITICAL: Cannot access saturation field at %p - PREVENTING BadVA CRASH\n", &tuning->saturation);
                return -EFAULT;
            }
            ...
            if ((unsigned long)&tuning->saturation < (unsigned long)tuning ||
                ...
                pr_info("CRITICAL: Saturation field address out of bounds - PREVENTING BadVA CRASH\n");
                return -EFAULT;
            }
...
static int apical_isp_core_ops_s_ctrl(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    if (!dev || !tuning) {
        pr_info("No ISP device or tuning data\n");
        return -EINVAL;
    }
...
//            }
        ...
        case 0x80000e2:  // Module Control - CRITICAL for ISP pipeline
            ...
            pr_info("apical_isp_core_ops_s_ctrl: Module control=%d (Binary Ninja reference)\n", ctrl->value);
        ...
        default:
            ...
            pr_info("apical_isp_core_ops_s_ctrl: Unhandled cmd=0x%x (Binary Ninja: return -1)\n", ctrl->cmd);
            ret = -EINVAL;
            break;
    }
...
int isp_core_tunning_unlocked_ioctl(struct file *file, unsigned int cmd, void __user *arg)
{
    ...
    if (!ourISPdev) {
        pr_info("*** TUNING IOCTL: ourISPdev is NULL - driver not fully initialized ***\n");
    ...
    dev = ourISPdev;
    if (!dev) {
        pr_info("isp_core_tunning_unlocked_ioctl: No ISP device available\n");
        return -ENODEV;
    }
    ...
    if (magic == 0x74 && (!ourISPdev || ourISPdev->tuning_enabled != 3)) {
        pr_info("isp_core_tunning_unlocked_ioctl: Tuning commands require explicit enable (cmd=0x%x)\n", cmd);
        return -ENODEV;
    }
        ...
        switch (cmd) {
            case 0xc008561c: /* ISP_CORE_S_CTRL - Set control */
                ...
                if (copy_from_user(&ctrl, (void __user *)arg, sizeof(ctrl))) {
                    pr_info("isp_core_tunning_unlocked_ioctl: Failed to copy control data from user\n");
                    return -EFAULT;
                }
                ...
                if (dev && dev->core_dev->tuning_data) {
                    ...
                    if ((unsigned long)tuning < 0x80000000 || (unsigned long)tuning >= 0xfffff000) {
                        pr_info("*** CRITICAL: tuning_data pointer 0x%p outside kernel memory - ABORTING ***\n", tuning);
                        return -EFAULT;
                    }

                    /* Validate alignment */
                    if ((unsigned long)tuning & 0x3) {
                        pr_info("*** CRITICAL: tuning_data pointer 0x%p not 4-byte aligned - ABORTING ***\n", tuning);
                        return -EFAULT;
                    }
                }
            ...
            case 0xc00c56c6: /* ISP_TUNING_ENABLE - Enable/disable tuning */
            {
                ...
                if (copy_from_user(&enable, (void __user *)arg, sizeof(enable))) {
                    pr_info("isp_core_tunning_unlocked_ioctl: Failed to copy tuning enable data from user\n");
                    return -EFAULT;
                }
                ...
                if (enable && ourISPdev && ourISPdev->tuning_enabled == 3) {
                    ...
                    pr_info("isp_core_tunning_unlocked_ioctl: Tuning enabled - Binary Ninja reference implementation\n");
                    ...
                    if (!tisp_par_ioctl) {
                        ...
                        if (tisp_par_ioctl) {
                            ...
                            pr_info("*** CRITICAL: Failed to allocate tuning parameter buffer ***\n");
                        }
                    }
            ...
            default:
                pr_info("isp_core_tunning_unlocked_ioctl: Unknown ISP core control command: 0x%x\n", cmd);
                ret = -EINVAL;
                break;
        }
    ...
    if (magic == 0x74) {
        ...
        if (!tisp_par_ioctl) {
            pr_info("tisp_code_tuning_ioctl: Global buffer not allocated\n");
            return -ENOMEM;
        }
                ...
                switch (cmd) {
                    case 0x20007400: { /* GET operation */
                        ...
                        if (!access_ok(VERIFY_READ, arg, 0x500c)) {
                            pr_info("tisp_code_tuning_ioctl: Access check failed for GET\n");
                            return -EFAULT;
                        }

                        /* SECURITY FIX: Validate user buffer is not in kernel space */
                        if ((unsigned long)arg >= 0x80000000) {
                            pr_info("tisp_code_tuning_ioctl: Invalid user buffer in kernel space\n");
                            return -EFAULT;
                        }
                        ...
                        if (!tisp_par_ioctl) {
                            ...
                            pr_info("tisp_code_tuning_ioctl: Buffer was freed during operation\n");
                            return -ENODEV;
                        }
                        ...
                        if (ret != 0) {
                            pr_info("tisp_code_tuning_ioctl: Copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        ...
                        if (param_type >= 0x19) {
                            ...
                            pr_info("tisp_code_tuning_ioctl: Invalid GET parameter type %d\n", param_type);
                            return -EINVAL;
                        }
                        ...
                        ret = copy_to_user((void __user *)s0_1, tisp_par_ioctl, 0x500c);
                        if (ret != 0) {
                            pr_info("tisp_code_tuning_ioctl: Final copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        break;
                    }
                    ...
                    case 0x20007401: { /* SET operation */
                        ...
                        if (!access_ok(VERIFY_WRITE, arg, 0x500c)) {
                            pr_info("tisp_code_tuning_ioctl: Access check failed for SET\n");
                            return -EFAULT;
                        }

                        /* SECURITY FIX: Validate user buffer is not in kernel space */
                        if ((unsigned long)arg >= 0x80000000) {
                            pr_info("tisp_code_tuning_ioctl: Invalid user buffer in kernel space for SET\n");
                            return -EFAULT;
                        }
                        ...
                        if (!tisp_par_ioctl) {
                            ...
                            pr_info("tisp_code_tuning_ioctl: Buffer was freed during SET operation\n");
                            return -ENODEV;
                        }
                        ...
                        if (ret != 0) {
                            pr_info("tisp_code_tuning_ioctl: Copy from user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        ...
                        if (param_type - 1 >= 0x18) {
                            pr_info("tisp_code_tuning_ioctl: Invalid SET parameter type %d\n", param_type);
                            return -EINVAL;
                        }
                    ...
                    case 0x20007403: { /* AE info get */
                        ...
                        if (!access_ok(VERIFY_WRITE, arg, 0x500c)) {
                            pr_info("tisp_code_tuning_ioctl: Access check failed for AE info get\n");
                            return -EFAULT;
                        }

                        ret = copy_to_user((void __user *)s0_1, tisp_par_ioctl, 0x500c);
                        if (ret != 0) {
                            pr_info("tisp_code_tuning_ioctl: AE info copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        break;
                    }
                    ...
                    case 0x20007404: { /* AE info set */
                        ...
                        if (!access_ok(VERIFY_READ, arg, 0x500c)) {
                            pr_info("tisp_code_tuning_ioctl: Access check failed for AE info set\n");
                            return -EFAULT;
                        }

                        ret = copy_from_user(tisp_par_ioctl, (void __user *)s0_1, 0x500c);
                        if (ret != 0) {
                            pr_info("tisp_code_tuning_ioctl: AE info copy from user failed: %d\n", ret);
                            return -EFAULT;
                        }
                    ...
                    case 0x20007406: { /* AWB info get */
                        ...
                        if (!access_ok(VERIFY_WRITE, arg, 0x500c)) {
                            pr_info("tisp_code_tuning_ioctl: Access check failed for AWB info get\n");
                            return -EFAULT;
                        }

                        ret = copy_to_user((void __user *)s0_1, tisp_par_ioctl, 0x500c);
                        if (ret != 0) {
                            pr_info("tisp_code_tuning_ioctl: AWB info copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        break;
                    }
                    ...
                    case 0x20007407: { /* AWB info set */
                        ...
                        if (!access_ok(VERIFY_READ, arg, 0x500c)) {
                            pr_info("tisp_code_tuning_ioctl: Access check failed for AWB info set\n");
                            return -EFAULT;
                        }

                        ret = copy_from_user(tisp_par_ioctl, (void __user *)s0_1, 0x500c);
                        if (ret != 0) {
                            pr_info("tisp_code_tuning_ioctl: AWB info copy from user failed: %d\n", ret);
                            return -EFAULT;
                        }
                    ...
                    case 0x20007408: { /* Special operation 1 */
                        ...
                        if (!access_ok(VERIFY_READ, arg, 0x500c)) {
                            pr_info("tisp_code_tuning_ioctl: Access check failed for special op 1\n");
                            return -EFAULT;
                        }

                        ret = copy_from_user(tisp_par_ioctl, (void __user *)s0_1, 0x500c);
                        if (ret != 0) {
                            pr_info("tisp_code_tuning_ioctl: Special op 1 copy from user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        ...
                        ret = copy_to_user((void __user *)s0_1, tisp_par_ioctl, 0x500c);
                        if (ret != 0) {
                            pr_info("tisp_code_tuning_ioctl: Special op 1 copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        break;
                    }
                    ...
                    case 0x20007409: { /* Special operation 2 */
                        ...
                        if (!access_ok(VERIFY_READ, arg, 0x500c)) {
                            pr_info("tisp_code_tuning_ioctl: Access check failed for special op 2\n");
                            return -EFAULT;
                        }

                        ret = copy_from_user(tisp_par_ioctl, (void __user *)s0_1, 0x500c);
                        if (ret != 0) {
                            pr_info("tisp_code_tuning_ioctl: Special op 2 copy from user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        ...
                        ret = copy_to_user((void __user *)s0_1, tisp_par_ioctl, 0x500c);
                        if (ret != 0) {
                            pr_info("tisp_code_tuning_ioctl: Special op 2 copy to user failed: %d\n", ret);
                            return -EFAULT;
                        }
                        break;
                    }
                    ...
                    default:
                        pr_info("tisp_code_tuning_ioctl: Unknown command in valid range: 0x%x\n", cmd);
                        return -EINVAL;
                }
    ...
    if (((cmd >> 8) & 0xff) != 0x74) {
        pr_info("tisp_code_tuning_ioctl: Command magic not 0x74: cmd=0x%x\n", cmd);
        return -EINVAL;
    } else {
        pr_info("tisp_code_tuning_ioctl: Command out of valid range: cmd=0x%x\n", cmd);
        return -EINVAL;
    }
}
...
int tisp_code_tuning_open(struct inode *inode, struct file *file)
{
    ...
    if (!tuning_buffer) {
        pr_info("tisp_code_tuning_open: Failed to allocate tuning buffer (0x%x bytes)\n", TUNING_PARAM_BUFFER_SIZE);
        return -ENOMEM;
    }

    /* CRITICAL: Verify alignment for MIPS - must be 4-byte aligned */
    if ((unsigned long)tuning_buffer & 0x3) {
        pr_info("CRITICAL: Tuning buffer not 4-byte aligned: %p\n", tuning_buffer);
        kfree(tuning_buffer);
        return -ENOMEM;
    }
...
long tisp_code_tuning_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    pr_info("tisp_code_tuning_ioctl: cmd=0x%x, arg=0x%lx\n", cmd, arg);
    ...
    if (((cmd >> 8) & 0xFF) == 0x74) {
            ...
            if ((cmd - 0x20007400) < 0xa) {
                ...
                switch (cmd) {
                    case 0x20007400: /* Get parameter configuration */
                    {
                        ...
                        if (!tisp_par_ioctl) {
                            pr_info("tisp_code_tuning_ioctl: Parameter buffer not allocated\n");
                            return -ENOMEM;
                        }

                        if (copy_from_user(tisp_par_ioctl, argp, 0x500c)) {
                            pr_info("tisp_code_tuning_ioctl: Failed to copy parameters from user\n");
                            return -EFAULT;
                        }
                        ...
                        pr_info("tisp_code_tuning_ioctl: Get parameter type %d\n", param_type);
                        ...
                        switch (param_type) {
                            ...
                            case 0x18: /* tisp_dn_mode_get */
                                ...
                                break;
                            default:
                                pr_info("tisp_code_tuning_ioctl: Unknown get parameter type %d\n", param_type);
                                ret = -EINVAL;
                                break;
                        }
                        ...
                        if (ret == 0) {
                            if (copy_to_user(argp, tisp_par_ioctl, 0x500c)) {
                                pr_info("tisp_code_tuning_ioctl: Failed to copy parameters to user\n");
                                return -EFAULT;
                            }
                        }
                    ...
                    case 0x20007401: /* Set parameter configuration */
                    {
                        ...
                        if ((param_type - 1) >= 0x18) {
                            pr_info("tisp_code_tuning_ioctl: Invalid parameter type %d\n", param_type);
                            return -EINVAL;
                        }
                        ...
                        switch (param_type) {
                            ...
                            case 0x15: /* tisp_awb_set_par_cfg */
                                ...
                                break;
                            case 0x16: /* Reserved */
                                pr_info("tisp_code_tuning_ioctl: Reserved parameter type 0x16\n");
                                ret = -EINVAL;
                                break;
                    ...
                    default:
                        pr_info("tisp_code_tuning_ioctl: Unknown tuning command 0x%x\n", cmd);
                        return -EINVAL;
                }
        ...
        pr_info("tisp_code_tuning_ioctl: Command out of range: 0x%x\n", cmd);
        return -EINVAL;
    }

    /* Binary Ninja: Handle non-0x74 commands */
    pr_info("tisp_code_tuning_ioctl: Invalid command family: 0x%x\n", cmd);
    return -EINVAL;
}
...
int apical_isp_ae_g_roi(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    pr_info("apical_isp_ae_g_roi: entry\n");

    buffer = kmalloc(0x384, GFP_KERNEL);
    if (buffer == NULL) {
        /* Binary Ninja: isp_printf(1, "not support the gpio mode!\n", $a2) */
        pr_info("apical_isp_ae_g_roi: not support the gpio mode!\n");
    ...
    if (result == 0) {
        ...
        pr_info("apical_isp_ae_g_roi: width/height/imagesize error\n");
    }
...
int apical_isp_ae_zone_g_ctrl(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    pr_info("apical_isp_ae_zone_g_ctrl: entry\n");
...
int apical_isp_af_zone_g_ctrl(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    pr_info("apical_isp_af_zone_g_ctrl: entry\n");
...
{
    if (!out_buf) {
        pr_info("tisp_g_wdr_en: NULL output buffer\n");
        return -EINVAL;
    }

    /* Binary Ninja: *arg1 = data_b2e74; return 0 */
    *(uint32_t *)out_buf = data_b2e74;
    pr_info("tisp_g_wdr_en: WDR enable = %d\n", data_b2e74);
    return 0;
}
...
int tisp_gb_param_array_get(int param_id, void *out_buf, int *size_buf)
{
    ...
    if ((param_id - 0x3f5) >= 0xa) {
        pr_info("tisp_gb_param_array_get: Invalid parameter ID 0x%x\n", param_id);
        return -1;
    }

    if (!out_buf || !size_buf) {
        pr_info("tisp_gb_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    switch (param_id) {
        ...
        case 0x3fe:  /* tisp_gb_blc_min */
            ...
            break;
        default:
            pr_info("tisp_gb_param_array_get: Unhandled parameter ID 0x%x\n", param_id);
            return -1;
    }
    ...
    *size_buf = data_size;
    pr_info("tisp_gb_param_array_get: ID=0x%x, size=%d\n", param_id, data_size);
    return 0;
}
...
int tisp_lsc_param_array_get(int param_id, void *out_buf, int *size_buf)
{
    ...
    if ((param_id - 0x54) >= 0xb) {
        pr_info("tisp_lsc_param_array_get: Invalid parameter ID 0x%x\n", param_id);
        return -1;
    }

    if (!out_buf || !size_buf) {
        pr_info("tisp_lsc_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    switch (param_id) {
        ...
        case 0x5e:  /* lsc_mean_en */
            ...
            break;
        default:
            pr_info("tisp_lsc_param_array_get: Unhandled parameter ID 0x%x\n", param_id);
            return -1;
    }
    ...
    memcpy(out_buf, source_ptr, data_size);
    *size_buf = data_size;
    pr_info("tisp_lsc_param_array_get: ID=0x%x, size=%d\n", param_id, data_size);
    return 0;
}
...
int tisp_wdr_param_array_get(int param_id, void *out_buf, int *size_buf)
{
    ...
    if ((param_id - 0x3ff) >= 0x33) {
        pr_info("tisp_wdr_param_array_get: Invalid parameter ID 0x%x\n", param_id);
        return -1;
    }

    if (!out_buf || !size_buf) {
        pr_info("tisp_wdr_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    memcpy(out_buf, source_ptr, data_size);
    *size_buf = data_size;
    pr_info("tisp_wdr_param_array_get: ID=0x%x, size=%d\n", param_id, data_size);
    return 0;
}
...
int tisp_wdr_param_array_get_extended(int param_id, void *out_buf, int *size_buf)
{
    ...
    switch (param_id) {
        ...
        case 0x431:  /* param_wdr_tool_control_array */
            ...
            break;
        default:
            pr_info("tisp_wdr_param_array_get_extended: Unhandled parameter ID 0x%x\n", param_id);
            return -1;
    }
    ...
    memcpy(out_buf, source_ptr, data_size);
    *size_buf = data_size;
    pr_info("tisp_wdr_param_array_get_extended: ID=0x%x, size=%d\n", param_id, data_size);
    return 0;
}
...
int tisp_dpc_param_array_get(int param_id, void *out_buf, int *size_buf)
{
    ...
    if ((param_id - 0xe6) >= 0x1f) {
        pr_info("tisp_dpc_param_array_get: Invalid parameter ID 0x%x\n", param_id);
        return -1;
    }

    if (!out_buf || !size_buf) {
        pr_info("tisp_dpc_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    switch (param_id) {
        ...
        case 0x104:  /* ctr_con_par_array */
            ...
            break;
        default:
            pr_info("tisp_dpc_param_array_get: Unhandled parameter ID 0x%x\n", param_id);
            return -1;
    }
    ...
    memcpy(out_buf, source_ptr, data_size);
    *size_buf = data_size;
    pr_info("tisp_dpc_param_array_get: ID=0x%x, size=%d\n", param_id, data_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_rdns_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_rdns_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_adr_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_adr_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_ccm_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_ccm_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_gamma_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_gamma_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_defog_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_defog_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_top_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = 0x60;
    ...
    pr_info("tisp_top_param_array_get: Copied sensor_info, size=0x60\n");
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_blc_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_blc_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_lsc_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_lsc_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_wdr_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_wdr_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_dpc_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_dpc_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_mdns_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_mdns_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_ydns_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_ydns_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_bcsh_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_bcsh_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_clm_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_clm_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_ysp_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_ysp_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_sdns_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_sdns_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_af_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = total_size;
    pr_info("tisp_af_get_par_cfg: Total size=%d\n", total_size);
    return 0;
}
...
int tisp_g_aeroi_weight(void *buffer)
{
    ...
    pr_info("tisp_g_aeroi_weight: entry, buffer=%p\n", buffer);

    tisp_ae_param_array_get(0x12, buffer, &var_10);

    if (var_10 == 0x384) {
        pr_info("tisp_g_aeroi_weight: success, size=0x384\n");
        return 0;
    }

    pr_info("tisp_g_aeroi_weight: bank no free\n");
...
int tisp_g_ae_zone_internal(void *buffer)
{
    ...
    pr_info("tisp_g_ae_zone_internal: entry, buffer=%p\n", buffer);
...
int tisp_g_af_zone_buffer(void *buffer)
{
    ...
    pr_info("tisp_g_af_zone_buffer: entry, buffer=%p\n", buffer);
...
int tisp_g_aezone_weight(void *buffer)
{
    ...
    pr_info("tisp_g_aezone_weight: entry, buffer=%p\n", buffer);

    tisp_ae_param_array_get(0x10, buffer, &var_10);

    if (var_10 == 0x384) {
        pr_info("tisp_g_aezone_weight: success, size=0x384\n");
        return 0;
    }

    pr_info("tisp_g_aezone_weight: Failed to allocate vic device\n");
    return -1;  /* Binary Ninja returns 0xffffffff */
}
...
int tisp_g_ae_hist(void *buffer)
{
    ...
    pr_info("tisp_g_ae_hist: entry, buffer=%p\n", buffer);
...
int tisp_ae_param_array_get(int param_type, void *buffer, int *size)
{
    ...
    if ((param_type - 1) >= 0x22) {
        pr_info("tisp_ae_param_array_get: Invalid parameter type %d\n", param_type);
        return -1;  /* Binary Ninja returns 0xffffffff */
    }
    ...
    switch (param_type) {
        ...
        case 0x22: /* ae1_comp_ev_list */
            ...
            break;
        default:
            pr_info("tisp_ae_param_array_get: Unhandled parameter type %d\n", param_type);
            return -1;
    }
    ...
    if (source_ptr && buffer && size) {
        ...
        *size = data_size;
        pr_info("tisp_ae_param_array_get: type=%d, size=%d\n", param_type, data_size);
        return 0;
    }
...
int isp_m0_chardev_release(struct inode *inode, struct file *file)
{
    ...
    if (ourISPdev && ourISPdev && ourISPdev->tuning_enabled == 3) {
...
static int tiziano_wdr_algorithm(void)
{
    ...
    if (!data_d94f8) {
        ...
        if (!data_d94f8) {
            pr_info("tiziano_wdr_algorithm: Failed to allocate output array\n");
            return -ENOMEM;
        }
...
static void tisp_ae1_process(void)
{
    ...
    pr_info("tisp_ae1_process: AE1 processing completed\n");
}
...
int tiziano_gamma_lut_parameter(void)
{
    ...
    if (!base_reg) {
        pr_info("tiziano_gamma_lut_parameter: Failed to map gamma registers\n");
        return -ENOMEM;
    }

    if (!tiziano_gamma_lut_now) {
        pr_info("tiziano_gamma_lut_parameter: No gamma LUT selected\n");
        iounmap(base_reg);
        return -EINVAL;
    }
...
{
    pr_info("tiziano_lsc_params_refresh: Refreshing LSC parameters\n");
    ...
    pr_info("tiziano_lsc_params_refresh: Updated LSC strength=0x%x, CT=%d\n", lsc_curr_str, data_9a450);
}
...
static int tiziano_ccm_lut_parameter(int32_t *ccm_data)
{
    ...
    if (!base_reg) {
        pr_info("tiziano_ccm_lut_parameter: Failed to map CCM registers\n");
        return -ENOMEM;
    }
...
{
    pr_info("tiziano_ccm_params_refresh: Refreshing CCM parameters\n");
...
{
    pr_info("tisp_ccm_ct_update: Updating CCM for color temperature changes (safe version)\n");
    ...
    if (ct_diff > data_c52f8) {  /* CT threshold check */
        pr_info("tisp_ccm_ct_update: Significant CT change detected (%d -> %d)\n",
...
{
    pr_info("tisp_ccm_ev_update: Updating CCM for exposure value changes (safe version)\n");
    ...
    if (ev_diff > data_c52f0) {  /* EV threshold check */
        pr_info("tisp_ccm_ev_update: Significant EV change detected (%u -> %u)\n",
...
int tiziano_ccm_init(void)
{
    ...
    int ret = jz_isp_ccm();
    if (ret) {
        pr_info("tiziano_ccm_init: Failed to initialize CCM: %d\n", ret);
        return ret;
    }
...
{
    pr_info("tiziano_sharpen_params_refresh: Refreshing sharpening parameters (simple version)\n");
...
static int tisp_sharpen_all_reg_refresh(void)
{
    ...
    if (!base_reg) {
        pr_info("tisp_sharpen_all_reg_refresh: Failed to map sharpening registers\n");
        return -ENOMEM;
    }
...
int tiziano_sharpen_init(void)
{
    ...
    if (ret) {
        pr_info("tiziano_sharpen_init: Failed to refresh sharpening parameters: %d\n", ret);
        return ret;
    }
...
{
    pr_info("tiziano_sdns_params_refresh: Refreshing SDNS parameters (simple version)\n");
    /* This is the simple version called from init - the enhanced version is elsewhere */
}
...
static int tisp_sdns_all_reg_refresh(void)
{
    ...
    if (!base_reg) {
        pr_info("tisp_sdns_all_reg_refresh: Failed to map SDNS registers\n");
        return -ENOMEM;
    }
...
{
    pr_info("tisp_sdns_intp_reg_refresh: Interpolated SDNS register refresh\n");
...
int tiziano_sdns_init(void)
{
    ...
    if (ret) {
        pr_info("tiziano_sdns_init: Failed to refresh SDNS parameters: %d\n", ret);
        return ret;
    }
...
int tiziano_mdns_init(uint32_t width, uint32_t height)
{
    ...
    if (!base_reg) {
        pr_info("tiziano_mdns_init: Failed to map MDNS registers\n");
        return -ENOMEM;
    }
...
{
    pr_info("tiziano_dpc_params_refresh: Refreshing DPC parameters\n");
    ...
    pr_info("tiziano_dpc_params_refresh: DPC parameters updated based on EV\n");
}
...
static int tisp_dpc_all_reg_refresh(void)
{
    ...
    if (!base_reg) {
        pr_info("tisp_dpc_all_reg_refresh: Failed to map DPC registers\n");
        return -ENOMEM;
    }
...
{
    pr_info("tiziano_adr_params_refresh: Refreshing ADR parameters\n");
    ...
    pr_info("tiziano_adr_params_refresh: ADR ratio updated to 0x%x\n", adr_ratio);
}
...
static int tisp_adr_set_params(void)
{
    ...
    if (!base_reg) {
        pr_info("tisp_adr_set_params: Failed to map ADR registers\n");
        return -ENOMEM;
    }
...
int tisp_adr_process(void)
{
    pr_info("tisp_adr_process: Processing ADR tone mapping\n");
    return 0;
}
...
int tiziano_adr_init(uint32_t width, uint32_t height)
{
    ...
    if (ret) {
        pr_info("tiziano_adr_init: Failed to set ADR parameters: %d\n", ret);
        return ret;
    }
...
int tiziano_gamma_init(void)
{
    pr_info("tiziano_gamma_init: Initializing Gamma processing\n");
...
int tiziano_lsc_init(void)
{
    pr_info("tiziano_lsc_init: Initializing LSC processing\n");
...
int tiziano_wdr_params_init(void)
{
    pr_info("tiziano_wdr_params_init: Initializing WDR parameters\n");
...
int tiziano_defog_params_init(void)
{
    pr_info("tiziano_defog_params_init: Initializing defog parameters\n");
...
int tiziano_s_awb_start(uint32_t r_gain, uint32_t b_gain)
{
    ...
    pr_info("tiziano_s_awb_start: Setting AWB gains R=%u, B=%u\n", r_gain, b_gain);
...
{
    pr_info("tiziano_g_awb_start: Getting AWB gains\n");
...
int tisp_s_wdr_init_en(int enable)
{
    ...
    pr_info("tisp_s_wdr_init_en: %s WDR initialization\n", enable ? "Enable" : "Disable");
...
int tisp_event_set_cb(int event_id, void *callback)
{
    ...
    if (event_id < 0 || event_id >= 32) {
        pr_info("tisp_event_set_cb: Invalid event ID %d\n", event_id);
        return -EINVAL;
    }
...
static irqreturn_t isp_irq_dispatcher(int irq, void *dev_id)
{
    ...
    if (!dev || !dev || !dev->core_regs) {
        pr_info("isp_irq_dispatcher: Invalid device or register base\n");
        return IRQ_NONE;
    }
...
static int isp_event_dispatcher(int event_id)
{
    ...
    if (event_id < 0 || event_id >= 32) {
        pr_info("isp_event_dispatcher: Invalid event ID %d\n", event_id);
        return -EINVAL;
    }
...
{
    pr_info("isp_trigger_event: Triggering event %d\n", event_id);
...
int tisp_event_process(void)
{
    ...
    if (s0_1 == data_b33b0) {
        ...
        pr_info("tisp_event_process: sensor type is BT1120!\n");
...
{
    pr_info("tisp_awb_deinit: Deinitializing AWB\n");
...
{
    pr_info("tisp_awb_algo_init: Initializing AWB algorithm (enable=%d)\n", enable);
...
{
    pr_info("tisp_awb_algo_deinit: Deinitializing AWB algorithm\n");
...
{
    pr_info("tisp_ae_deinit: Deinitializing AE\n");
...
{
    pr_info("tisp_ae_algo_init: Initializing AE algorithm (enable=%d)\n", enable);
...
{
    pr_info("tisp_ae_algo_deinit: Deinitializing AE algorithm\n");
...
int tisp_tgain_update(void)
{
    pr_info("tisp_tgain_update: Updating total gain\n");
    ...
    if (ourISPdev && ourISPdev && ourISPdev->core_dev->tuning_data) {
        ...
        if (sensor && sensor->sd.ops &&
            ...
            if (sensor_ret == 0) {
                ...
                pr_info("tisp_again_update: Sensor I2C gain update FAILED: %d\n", sensor_ret);
            }
        } else {
            pr_info("tisp_again_update: No sensor available for I2C communication\n");
        }
...
int tisp_ev_update(void)
{
    pr_info("tisp_ev_update: Updating exposure value\n");
...
int tisp_ct_update(void)
{
    pr_info("tisp_ct_update: Updating color temperature\n");
...
int tisp_ae_ir_update(void)
{
    pr_info("tisp_ae_ir_update: Updating AE IR parameters\n");
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_ccm_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *size_buf = 0x24;
    pr_info("tisp_ccm_param_array_get: ID=0x%x, size=0x24\n", param_id);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_gamma_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *size_buf = 0x102;
    pr_info("tisp_gamma_param_array_get: ID=0x%x, size=0x102\n", param_id);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_sharpen_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    memset(out_buf, 0, 0x24);
    *size_buf = 0x24;
    pr_info("tisp_sharpen_param_array_get: ID=0x%x, size=0x24\n", param_id);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_mdns_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    memset(out_buf, 0, 0x24);
    *size_buf = 0x24;
    pr_info("tisp_mdns_param_array_get: ID=0x%x, size=0x24\n", param_id);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_defog_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    memset(out_buf, 0, 0x24);
    *size_buf = 0x24;
    pr_info("tisp_defog_param_array_get: ID=0x%x, size=0x24\n", param_id);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_rdns_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    memset(out_buf, 0, 0x24);
    *size_buf = 0x24;
    pr_info("tisp_rdns_param_array_get: ID=0x%x, size=0x24\n", param_id);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_clm_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    memset(out_buf, 0, 0x24);
    *size_buf = 0x24;
    pr_info("tisp_clm_param_array_get: ID=0x%x, size=0x24\n", param_id);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_adr_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    memset(out_buf, 0, 0x24);
    *size_buf = 0x24;
    pr_info("tisp_adr_param_array_get: ID=0x%x, size=0x24\n", param_id);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_ydns_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    memset(out_buf, 0, 0x24);
    *size_buf = 0x24;
    pr_info("tisp_ydns_param_array_get: ID=0x%x, size=0x24\n", param_id);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_af_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    memset(out_buf, 0, 0x24);
    *size_buf = 0x24;
    pr_info("tisp_af_param_array_get: ID=0x%x, size=0x24\n", param_id);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_sdns_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    memset(out_buf, 0, 0x24);
    *size_buf = 0x24;
    pr_info("tisp_sdns_param_array_get: ID=0x%x, size=0x24\n", param_id);
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_bcsh_param_array_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    memset(out_buf, 0, 0x24);
    *size_buf = 0x24;
    pr_info("tisp_bcsh_param_array_get: ID=0x%x, size=0x24\n", param_id);
    return 0;
}
...
struct tx_isp_sensor *tx_isp_get_sensor(void)
{
    ...
    if (!ourISPdev) {
        pr_info("*** tx_isp_get_sensor: ourISPdev is NULL ***\n");
        return NULL;
    }
    ...
    if (sd && sd->ops && sd->ops->sensor) {
        ...
        if (sd->pdev && sd->pdev->name && strcmp(sd->pdev->name, "isp-m0") != 0) {
            ...
            if (!sensor) {
                ...
                if (!sensor) {
                    pr_info("*** tx_isp_get_sensor: Failed to allocate sensor structure ***\n");
                    return NULL;
                }
    ...
    pr_info("*** tx_isp_get_sensor: No real sensor found in subdev array ***\n");
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_ae_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = 0x100;
    pr_info("tisp_ae_get_par_cfg: Returned AE config, size=0x100\n");
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_hldc_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = 0x80;
    pr_info("tisp_hldc_get_par_cfg: Returned HLDC config, size=0x80\n");
    return 0;
}
...
{
    if (!out_buf || !size_buf) {
        pr_info("tisp_awb_get_par_cfg: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = 0x200;
    pr_info("tisp_awb_get_par_cfg: Returned AWB config, size=0x200\n");
    return 0;
}
...
{
    if (!reg_val || !size_buf) {
        pr_info("tisp_reg_map_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = sizeof(uint32_t);
    pr_info("tisp_reg_map_get: Register 0x%x = 0x0\n", reg_addr);
    return 0;
}
...
int tisp_dn_mode_get(void *mode_buf, void *size_buf)
{
    if (!mode_buf || !size_buf) {
        pr_info("tisp_dn_mode_get: NULL buffer pointers\n");
        return -EINVAL;
    }
    ...
    *(int *)size_buf = sizeof(uint32_t);
    pr_info("tisp_dn_mode_get: DN mode = 0\n");
    return 0;
}
...
int tisp_code_create_tuning_node(void)
{
    ...
    if (tuning_major == 0) {
        ...
        if (ret < 0) {
            pr_info("tisp_code_create_tuning_node: Failed to allocate chrdev region: %d\n", ret);
            return ret;
        }
        ...
        if (ret < 0) {
            pr_info("tisp_code_create_tuning_node: Failed to register chrdev region: %d\n", ret);
            return ret;
        }
    ...
    if (ret < 0) {
        pr_info("tisp_code_create_tuning_node: Failed to add cdev: %d\n", ret);
    ...
    if (IS_ERR(tuning_class)) {
        ...
        pr_info("tisp_code_create_tuning_node: Failed to create class: %d\n", ret);
    ...
    if (device_create(tuning_class, NULL, tuning_devno, NULL, "isp-m0") == NULL) {
        pr_info("tisp_code_create_tuning_node: Failed to create device\n");
...
int tisp_s_Gamma(void *gamma_data)
{
    ...
    if (!gamma_data) {
        pr_info("tisp_s_Gamma: NULL gamma data\n");
        return -EINVAL;
    }
...
int tisp_s_adr_enable(int enable)
{
    ...
    if (enable == 1) {
        ...
        pr_info("tisp_s_adr_enable: Invalid enable value %d\n", enable);
        return -EINVAL;
    }
...
int tisp_s_ae_attr(void *ae_attr_data)
{
    ...
    if (!ae_attr_data) {
        pr_info("tisp_s_ae_attr: NULL AE attribute data\n");
        return -EINVAL;
    }
...
int tisp_s_ae_hist(void *hist_data)
{
    ...
    if (!hist_data) {
        pr_info("tisp_s_ae_hist: NULL histogram data\n");
        return -EINVAL;
    }
...
void *isp_core_tuning_init(void *arg1)
{
    ...
    if (!tuning_data) {
        pr_info("isp_core_tuning_init: Failed to allocate tuning data structure\n");
        return NULL;
    }

    /* SAFE: Verify alignment for MIPS - must be 4-byte aligned */
    if ((unsigned long)tuning_data & 0x3) {
        pr_info("CRITICAL: Tuning data not 4-byte aligned: %p\n", tuning_data);
        kfree(tuning_data);
        return NULL;
    }
...
int isp_core_tuning_event(void *arg1, int arg2)
{
    ...
    if (!tuning_data) {
        pr_info("isp_core_tuning_event: NULL tuning data pointer\n");
        return -EINVAL;
    }
...
void check_csi_error(void)
{
    ...
    if (!isp_dev || !isp_dev->csi_dev) {
        pr_info("check_csi_error: No CSI device available\n");
        return;
    }
    ...
    while (1) {
        ...
        if (csi_regs) {
            ...
            if (csi_err1 != 0) {
                pr_info("CSI Error 1 (0x20): 0x%x\n", csi_err1);
            }

            /* Binary Ninja: if ($s3_1 != 0) isp_printf(...) */
            if (csi_err2 != 0) {
                pr_info("CSI Error 2 (0x24): 0x%x\n", csi_err2);
            }
        }
...
int tiziano_deflicker_expt(uint32_t flicker_t, uint32_t param2, uint32_t param3, uint32_t param4, uint32_t *lut_array, uint32_t *nodes_count)
{
    pr_info("tiziano_deflicker_expt: flicker_t=%u, param2=%u, param3=%u, param4=%u\n",
             flicker_t, param2, param3, param4);

    if (!lut_array || !nodes_count) {
        pr_info("tiziano_deflicker_expt: NULL pointer parameters\n");
        return -EINVAL;
    }
    ...
    *nodes_count = *nodes_count - 1;
    ...
    pr_info("tiziano_deflicker_expt: Generated %u LUT entries\n", *nodes_count);
    return 0;
}
...
int tiziano_ae_params_refresh(void)
{
    pr_info("tiziano_ae_params_refresh: Refreshing AE parameters\n");
    ...
    pr_info("tiziano_ae_params_refresh: AE parameters refreshed\n");
    return 0;
}
...
void *tiziano_ae_para_addr(void)
{
    pr_info("tiziano_ae_para_addr: Setting up AE parameter addresses\n");
    ...
    pr_info("tiziano_ae_para_addr: AE parameter addresses configured\n");
...
{
    pr_info("tisp_set_sensor_integration_time: Setting integration time to %u\n", time);

    extern struct tx_isp_dev *ourISPdev;
    if (!ourISPdev) {
        pr_info("tisp_set_sensor_integration_time: No ISP device available\n");
        return;
    }
...
void tisp_set_sensor_analog_gain(void)
{
    ...
    pr_info("tisp_set_sensor_analog_gain: Setting analog gain\n");
    ...
    pr_info("tisp_set_sensor_analog_gain: Calculated gain = %u\n", final_gain);
}
...
void tisp_set_sensor_integration_time_short(uint32_t time)
{
    ...
    pr_info("tisp_set_sensor_integration_time_short: Setting short integration time to %u\n", time);
...
int data_b2eec(uint32_t time, void **var_ptr)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_info("data_b2eec: No sensor available\n");
...
int data_b2ef0(uint32_t time, void **var_ptr)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_info("data_b2ef0: No sensor available\n");
        if (var_ptr) *var_ptr = NULL;
        return time;
    }
...
int data_b2ef4(uint32_t param, int flag)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_info("data_b2ef4: No sensor available\n");
        return -ENODEV;
    }
...
int data_b2ef8(uint32_t param, int flag)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_info("data_b2ef8: No sensor available\n");
        return -ENODEV;
    }
...
uint32_t data_b2ee0(uint32_t log_val, int16_t *var_ptr)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_info("data_b2ee0: No sensor available\n");
...
uint32_t data_b2ee4(uint32_t log_val, void **var_ptr)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_info("data_b2ee4: No sensor available\n");
        if (var_ptr) *var_ptr = NULL;
        return log_val;
    }
...
int data_b2f04(uint32_t param, int flag)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_info("data_b2f04: No sensor available\n");
        return -ENODEV;
    }
...
int data_b2f08(uint32_t param, int flag)
{
    ...
    struct tx_isp_sensor *sensor = tx_isp_get_sensor();
    if (!sensor) {
        pr_info("data_b2f08: No sensor available\n");
        return -ENODEV;
    }
