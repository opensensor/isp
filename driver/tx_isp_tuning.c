...
uint32_t system_reg_read(u32 reg)
{
    ...
    if (!ourISPdev || !ourISPdev->vic_dev->vic_regs) {
        return 0;
    }
...
//                    return -EFAULT;
    ...
    switch (ctrl->cmd) {
        ...
        case 0x8000164:  // ISP_CTRL_BYPASS
            ctrl->value = ourISPdev->core_dev->bypass_enabled;
            break;
...
int isp_core_tunning_unlocked_ioctl(struct file *file, unsigned int cmd, void __user *arg)
{
    ...
    if (magic == 0x56 && ourISPdev->core_dev->tuning_enabled != 3 && !auto_init_done) {
    ...
    if (magic == 0x74 && ourISPdev->core_dev->tuning_enabled != 3) {
    ...
    if (magic == 0x56) {
        ...
        switch (cmd) {
            ...
            case 0xc00c56c6: /* ISP_TUNING_ENABLE - Enable/disable tuning */
            {
                ...
                if (enable && ourISPdev->core_dev->tuning_enabled == 3) {
...
int tisp_rdns_param_array_get(int param_id, void *out_buf, int *size_buf)
{
    pr_debug("tisp_rdns_param_array_get: ID=0x%x (stub)\n", param_id);
    if (out_buf && size_buf) {
        *size_buf = 0;
    }
    return 0;
...
int tisp_adr_param_array_get(int param_id, void *out_buf, int *size_buf)
{
    pr_debug("tisp_adr_param_array_get: ID=0x%x (stub)\n", param_id);
    if (out_buf && size_buf) {
        *size_buf = 0;
    }
    return 0;
...
int isp_m0_chardev_release(struct inode *inode, struct file *file)
{
    ...
    if (ourISPdev && ourISPdev->core_dev->tuning_enabled == 3) {
