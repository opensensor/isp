...
int tiziano_ae_init(uint32_t height, uint32_t width, uint32_t fps)
{
    ...
    *data_d04c4 = height;
    ...
    /* These callbacks process frame statistics and are required for proper operation */
    /* Bits 8 (0x100) and 10 (0x400) are being set in interrupt status but callbacks were disabled */

    pr_info("*** CRITICAL FIX: Registering ISP interrupt callbacks for AE/AWB/AF ***\n");
    system_irq_func_set(0x08, ae0_interrupt_static);    /* Index 8: For status 0x100 - AE0 processing */
    system_irq_func_set(0x0a, ae0_interrupt_static);    /* Index 10: For status 0x400 - AE0 static */
