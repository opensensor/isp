...
/* AE interrupt wrapper functions - SAFE implementations with parameter validation */
irqreturn_t ae0_interrupt_hist_wrapper(int irq, void *dev_id) {
    ...
    if (!dev_id || (uintptr_t)dev_id < 0x80000000 || (uintptr_t)dev_id > 0x9fffffff) {
        pr_err("ae0_interrupt_hist_wrapper: Invalid dev_id=%p for IRQ %d\n", dev_id, irq);
        return IRQ_HANDLED;
    }
    ...
    extern struct tx_isp_dev *ourISPdev;
    if (dev_id != ourISPdev) {
        pr_err("ae0_interrupt_hist_wrapper: dev_id=%p != ourISPdev=%p - proceeding with caution\n", dev_id, ourISPdev);
    }
...
irqreturn_t ae0_interrupt_static_wrapper(int irq, void *dev_id) {
    /* CRITICAL SAFETY: Validate dev_id is a tx_isp_dev, not tx_isp_subdev */
    if (!dev_id || (uintptr_t)dev_id < 0x80000000 || (uintptr_t)dev_id > 0x9fffffff) {
        pr_err("ae0_interrupt_static_wrapper: Invalid dev_id=%p for IRQ %d\n", dev_id, irq);
        return IRQ_HANDLED;
    }
    ...
    extern struct tx_isp_dev *ourISPdev;
    if (dev_id != ourISPdev) {
        pr_err("ae0_interrupt_static_wrapper: dev_id=%p != ourISPdev=%p - proceeding with caution\n", dev_id, ourISPdev);
    }
...
irqreturn_t ae1_interrupt_hist_wrapper(int irq, void *dev_id) {
    /* CRITICAL SAFETY: Validate dev_id is a tx_isp_dev, not tx_isp_subdev */
    if (!dev_id || (uintptr_t)dev_id < 0x80000000 || (uintptr_t)dev_id > 0x9fffffff) {
        pr_err("ae1_interrupt_hist_wrapper: Invalid dev_id=%p for IRQ %d\n", dev_id, irq);
        return IRQ_HANDLED;
    }
    ...
    extern struct tx_isp_dev *ourISPdev;
    if (dev_id != ourISPdev) {
        pr_err("ae1_interrupt_hist_wrapper: dev_id=%p != ourISPdev=%p - proceeding with caution\n", dev_id, ourISPdev);
    }
...
irqreturn_t ae1_interrupt_static_wrapper(int irq, void *dev_id) {
    /* CRITICAL SAFETY: Validate dev_id is a tx_isp_dev, not tx_isp_subdev */
    if (!dev_id || (uintptr_t)dev_id < 0x80000000 || (uintptr_t)dev_id > 0x9fffffff) {
        pr_err("ae1_interrupt_static_wrapper: Invalid dev_id=%p for IRQ %d\n", dev_id, irq);
        return IRQ_HANDLED;
    }
    ...
    extern struct tx_isp_dev *ourISPdev;
    if (dev_id != ourISPdev) {
        pr_err("ae1_interrupt_static_wrapper: dev_id=%p != ourISPdev=%p - proceeding with caution\n", dev_id, ourISPdev);
    }
            ...
            case 0xc00c56c6: /* ISP_TUNING_ENABLE - Enable/disable tuning */
            {
                ...
                if (enable && ourISPdev->core_dev && ourISPdev->core_dev->tuning_enabled == 3) {
                    ...
                    break;
                            ...
                            extern uint32_t vic_start_ok;
                            if (vic_start_ok == 1) {
                                pr_info("*** TUNING: Skipping CSI PHY register maintenance during streaming to prevent corruption ***\n");
...
int tisp_channel_start(int channel, void *attr)
{
    ...
    pr_info("tisp_channel_start: Starting channel %d\n", channel);
