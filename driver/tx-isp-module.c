                ...
                if (ret && ret != 0xfffffdfd) {  // 0xfffffdfd = already initialized
                    ...
                    if (isp_dev->vic_regs) {
                        ...
                        /* C90 compliance: declare all variables at block start */
                        {
                            ...
                            /* Step 5: Stream control register calculation from Binary Ninja */
                            if (num_buffers < 8) {
                                mdma_stream_ctrl_base = (num_buffers << 16) | 0x80000020;
                            } else {
                                mdma_stream_ctrl_base = 0x80080020;
                            }
                            
                            mdma_stream_ctrl = mdma_stream_ctrl_base | pixel_format;
                            writel(mdma_stream_ctrl, vic_regs + 0x300);
                            wmb();
                            
                            pr_info("vic_mdma_enable: reg 0x300 = 0x%x (stream control, buffers=%d, fmt=%d)\n",
                                   mdma_stream_ctrl, num_buffers, pixel_format);
                            ...
                            if (read_val == test_val) {
                                ...
                                while (unlock_timeout > 0) {
                                    unlock_vic_status = readl(vic_regs + 0x0);
                                    if (unlock_vic_status == 0) break;
                                    udelay(10);
                                    unlock_timeout--;
                                }
