...
irqreturn_t isp_vic_interrupt_service_routine(void *arg1)
{
    ...
    /* CRITICAL FIX: Use safe struct member access instead of dangerous raw pointer arithmetic */
    /* The offset 0x13c was causing kernel panics - use proper struct member */
    vic_irq_enable_flag = &vic_dev->irq_enabled;
    ...
    if (vic_start_ok != 0) {
        ...
        if ((v1_7 & 1) != 0) {
            ...
            /* CRITICAL: Frame counter is already updated above - no need for duplicate increment */
...
int tx_isp_video_s_stream(struct tx_isp_dev *arg1, int arg2)
{
    ...
    for (i = 0; i != 0x10; ) {
        ...
        if (a0 != 0) {
            ...
            /* $a0 + 0xc4 = ops, *($a0 + 0xc4) = ops pointer, *(*($a0 + 0xc4) + 4) = ops->video */
            struct tx_isp_subdev_video_ops *v0_3 = NULL;
            if (a0->ops && a0->ops->video) {
                v0_3 = a0->ops->video;
            }

            /* Binary Ninja: if ($v0_3 == 0) i += 1 */
            if (v0_3 == 0) {
                i += 1;
            } else {
                /* Binary Ninja: int32_t $v0_4 = *$v0_3 */
                /* $v0_3 is video ops, *$v0_3 is s_stream function pointer */
                int (*v0_4)(struct tx_isp_subdev *, int) = v0_3->s_stream;

                /* Binary Ninja: if ($v0_4 == 0) i += 1 */
                if (v0_4 == 0) {
                    i += 1;
                } else {
                    ...
                    /* Binary Ninja: if (result == 0) i += 1 */
                    if (result == 0) {
                        pr_info("tx_isp_video_s_stream: Subdev %d s_stream returned success\n", i);
                        i += 1;
                    } else {
                        /* Binary Ninja: if (result != 0xfffffdfd) */
                        if (result != -ENOIOCTLCMD) {  /* 0xfffffdfd = -515 = -ENOIOCTLCMD */
                            pr_err("tx_isp_video_s_stream: Subdev %d s_stream failed: %d\n", i, result);

                            /* Binary Ninja rollback: void* $s0_1 = arg1 + (i << 2) */
                            /* Roll back all previous subdevices */
                            struct tx_isp_subdev **s0_1 = &arg1->subdevs[i];

                            /* Binary Ninja: while (arg1 != $s0_1) */
                            while (&arg1->subdevs[0] != s0_1) {
                                /* Move to previous subdev: $s0_1 -= 4 (pointer arithmetic) */
                                s0_1--;

                                /* Binary Ninja: void* $a0_1 = *($s0_1 + 0x38) */
                                /* But $s0_1 is already pointing to subdevs array, so just dereference */
                                struct tx_isp_subdev *a0_1 = *s0_1;

                                /* Binary Ninja: if ($a0_1 == 0) $s0_1 -= 4 */
                                if (a0_1 == 0) {
                                    continue; /* Already decremented s0_1 */
                                } else {
                                    /* Binary Ninja: int32_t* $v0_6 = *(*($a0_1 + 0xc4) + 4) */
                                    struct tx_isp_subdev_video_ops *v0_6 = NULL;
                                    if (a0_1->ops && a0_1->ops->video) {
                                        v0_6 = a0_1->ops->video;
                                    }

                                    /* Binary Ninja: if ($v0_6 == 0) $s0_1 -= 4 */
                                    if (v0_6 == 0) {
                                        continue; /* Already decremented s0_1 */
                                    } else {
...
next_iteration:
    ...
    /* Binary Ninja: return 0 */
    pr_info("*** tx_isp_video_s_stream: All subdevs processed successfully ***\n");
    return 0;
}
        ...
        if (ourISPdev && ourISPdev->vic_dev) {
            ...
            if (!vic_dev->streaming) {
                ...
                if (vic_dev->vic_regs) {
                    ...
                    // STEP 3: Wait for VIC ready state with proper timeout handling
                    while ((vic_status = ioread32(vic_dev->vic_regs + 0x0)) != 0 && timeout-- > 0) {
                        cpu_relax();
                        udelay(1); // Add small delay to prevent tight loop
                    }

                    pr_info("Channel %d: VIC ready wait complete (status=0x%x, timeout=%d)\n",
                           channel, vic_status, timeout);

                    // CRITICAL FIX: Check if timeout occurred - this prevents hard hangs
                    if (timeout <= 0 && vic_status != 0) {
                        pr_err("*** Channel %d: VIC TIMEOUT - Hardware not responding! status=0x%x ***\n",
                               channel, vic_status);
                        pr_err("*** Channel %d: VIC hardware may be in bad state - aborting streaming ***\n", channel);
                        spin_unlock_irqrestore(&vic_dev->buffer_lock, flags);
                        state->streaming = false;
                        return -ETIMEDOUT;
                    }
                ...
                /* REMOVED: Frame worker start - NOT in reference driver */
                /* Reference driver relies on hardware interrupts for frame processing */
