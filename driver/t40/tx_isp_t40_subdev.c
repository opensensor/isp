...
#define TX_ISP_T40_GRAPH_SUBDEVS_OFFSET 0x3cU
#define TX_ISP_T40_GRAPH_SUBDEV_COUNT   16U
#define TX_ISP_T40_STATE_QUEUE_OFFSET   0x1fcU
#define TX_ISP_T40_STATE_FLAGS_OFFSET   0x218U

static int tx_isp_t40_valid_pointer(const void *pointer)
{
	unsigned long value = (unsigned long)pointer;

	return value >= 0x80000000UL && value < 0xfffff001UL &&
	       !(value & (sizeof(unsigned int) - 1));
}

static void *tx_isp_t40_subdev_at(void *graph, unsigned int index)
{
	void *subdev = *(void **)((char *)graph +
		TX_ISP_T40_GRAPH_SUBDEVS_OFFSET + index * sizeof(void *));

	return tx_isp_t40_valid_pointer(subdev) ? subdev : 0;
}

static const char *tx_isp_t40_subdev_name(void *subdev)
{
	const char *name = *(const char **)((char *)subdev +
		TX_ISP_ABI_SUBDEV_NAME_OFFSET);

	return tx_isp_t40_valid_pointer(name) ? name : 0;
}

static unsigned int tx_isp_t40_pad_count(void *subdev, unsigned int type)
{
...
static void *tx_isp_t40_pad_base(void *subdev, unsigned int type)
{
	unsigned int offset = type == TX_ISP_SUBDEV_PAD_OUTPUT ?
	...
	return tx_isp_t40_valid_pointer(pads) ? pads : 0;
}

static const struct tx_isp_subdev_resolver_ops tx_isp_t40_subdev_ops = {
	.subdev_count = TX_ISP_T40_GRAPH_SUBDEV_COUNT,
	.pad_stride = TX_ISP_ABI_PAD_STRIDE,
	.subdev_at = tx_isp_t40_subdev_at,
	.subdev_name = tx_isp_t40_subdev_name,
	.pad_count = tx_isp_t40_pad_count,
	.pad_base = tx_isp_t40_pad_base,
};
	...
	enum tx_isp_subdev_resolve_status *status)
{
	...
	return (unsigned long)tx_isp_subdev_resolve_pad(
		(void *)graph, &common_descriptor, &tx_isp_t40_subdev_ops,
		status);
}

static void *tx_isp_t40_remote_pad(void *local_pad)
{
	return *(void **)((char *)local_pad + TX_ISP_ABI_PAD_LINK_OFFSET +
			 TX_ISP_ABI_LINK_SINK_OFFSET);
}

static unsigned long tx_isp_t40_event_handler(void *remote_pad)
{
	return *(unsigned int *)((char *)remote_pad +
				TX_ISP_ABI_PAD_EVENT_OFFSET);
}

static const struct tx_isp_remote_event_ops tx_isp_t40_remote_event_ops = {
	.remote_pad = tx_isp_t40_remote_pad,
	.event_handler = tx_isp_t40_event_handler,
};
	...
	struct tx_isp_remote_event_target *target)
{
	return tx_isp_resolve_remote_event(
		local_pad, &tx_isp_t40_remote_event_ops, pointer_valid, target);
}
...
int tx_isp_t40_subdev_state_ready(void *object)
{
	...
	queue_next = *(unsigned int *)((char *)object +
				      TX_ISP_T40_STATE_QUEUE_OFFSET);
	state = *(unsigned char *)((char *)object +
				  TX_ISP_T40_STATE_FLAGS_OFFSET);
	return tx_isp_subdev_state_ready(
		(unsigned long)object, queue_next,
		(unsigned long)((char *)object + TX_ISP_T40_STATE_QUEUE_OFFSET),
		state);
}

/* Build the shared resolver, remote-event route, and state policy into T40. */
