                                     ...
                                     size_t count, loff_t *ppos)
{
    ...
    printk(KERN_ALERT "ISP W02 proc command: %s\n", cmd);
    
    /* Handle common ISP commands that userspace might send */
    if (strncmp(cmd, "snapraw", 7) == 0) {
        printk(KERN_ALERT "ISP W02 snapraw command received\n");
        /* Handle raw snapshot command */
    } else if (strncmp(cmd, "enable", 6) == 0) {
        printk(KERN_ALERT "ISP W02 enable command received\n");
        if (isp)
            isp->streaming_enabled = true;
    } else if (strncmp(cmd, "disable", 7) == 0) {
        printk(KERN_ALERT "ISP W02 disable command received\n");
        if (isp)
            isp->streaming_enabled = false;
    }
                                    ...
                                    size_t count, loff_t *ppos)
{
    ...
    printk(KERN_ALERT "ISP FS proc command: %s\n", cmd);
    
    /* Handle FS-specific commands */
    if (strncmp(cmd, "enable", 6) == 0) {
        printk(KERN_ALERT "ISP FS enable command received\n");
        if (isp)
            isp->streaming_enabled = true;
    } else if (strncmp(cmd, "disable", 7) == 0) {
        printk(KERN_ALERT "ISP FS disable command received\n");
        if (isp)
            isp->streaming_enabled = false;
    }
                                    ...
                                    size_t count, loff_t *ppos)
{
    ...
    printk(KERN_ALERT "ISP M0 proc command: %s\n", cmd);
    
    /* Handle M0-specific commands */
    if (strncmp(cmd, "enable", 6) == 0) {
        printk(KERN_ALERT "ISP M0 enable command received\n");
        if (isp)
            isp->streaming_enabled = true;
    } else if (strncmp(cmd, "disable", 7) == 0) {
        printk(KERN_ALERT "ISP M0 disable command received\n");
        if (isp)
            isp->streaming_enabled = false;
    }
...
static struct proc_dir_entry *get_or_create_proc_dir(const char *name, struct proc_dir_entry *parent, bool *created)
{
    ...
    *created = false;
    ...
    if (dir) {
        ...
        *created = true;
        printk(KERN_ALERT "Created or accessed proc directory: %s\n", name);
        return dir;
    }
...
int tx_isp_create_proc_entries(struct tx_isp_dev *isp)
{
    ...
    printk(KERN_ALERT "*** tx_isp_create_proc_entries: Creating proc entries to match reference driver ***\n");
    ...
    printk(KERN_ALERT "Created proc entry: /proc/jz/isp/isp-w00\n");
    ...
    printk(KERN_ALERT "Created proc entry: /proc/jz/isp/isp-w01\n");
    ...
    printk(KERN_ALERT "*** CREATED PROC ENTRY: /proc/jz/isp/isp-w02 (CRITICAL FOR VIC FUNCTIONALITY) ***\n");
    ...
    printk(KERN_ALERT "*** CREATED PROC ENTRY: /proc/jz/isp/isp-fs (CRITICAL FOR FS FUNCTIONALITY) ***\n");
    ...
    printk(KERN_ALERT "*** CREATED PROC ENTRY: /proc/jz/isp/isp-m0 (CRITICAL FOR M0 FUNCTIONALITY) ***\n");
    ...
    printk(KERN_ALERT "Created proc entry: /proc/jz/isp/csi\n");
    ...
    printk(KERN_ALERT "Created proc entry: /proc/jz/isp/vic\n");

    printk(KERN_ALERT "*** ALL PROC ENTRIES CREATED SUCCESSFULLY - MATCHES REFERENCE DRIVER LAYOUT ***\n");
    printk(KERN_ALERT "*** /proc/jz/isp/ now contains: isp-w00, isp-w01, isp-w02, isp-fs, isp-m0, csi, vic ***\n");
...
void tx_isp_remove_proc_entries(void)
{
    ...
    printk(KERN_ALERT "*** tx_isp_remove_proc_entries: Cleaning up proc entries ***\n");
    ...
    printk(KERN_ALERT "All proc entries removed\n");
}
