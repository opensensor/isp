...
/* Expose current pixel format to VIC so it can set proper stride/format */
                                     ...
                                     struct v4l2_format *f)
{
    ...
    /* Update global current pixfmt for VIC configuration */
    tx_isp_current_pixfmt = f->fmt.pix.pixelformat;

    pr_info("*** Channel %d: S_FMT SUCCESS - Format configured %dx%d pixfmt=0x%x ***\n",
            dev->channel_num, f->fmt.pix.width, f->fmt.pix.height, tx_isp_current_pixfmt);
