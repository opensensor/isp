                          ...
                          void *driver_data)
{
    ...
    if (!desc || !pdev) {
        pr_info("tx_isp_subdev_register: Invalid parameters\n");
        return -EINVAL;
    }
    ...
    if (subdev_count >= ISP_MAX_SUBDEVS) {
        pr_info("tx_isp_subdev_register: Too many subdevices\n");
        ret = -ENOSPC;
        goto unlock;
    }
...
int tx_isp_create_subdev_graph(struct tx_isp_dev *isp)
{
    ...
    if (!isp) {
        pr_info("tx_isp_create_subdev_graph: Invalid ISP device\n");
        return -EINVAL;
    }
    ...
    if (subdev_count == 0) {
        ...
        if (ret < 0) {
            pr_info("Failed to create basic pipeline: %d\n", ret);
        }
        goto unlock;
    }
    ...
    for (i = 0; i < subdev_count; i++) {
        ...
        if (runtime->desc->type == TX_ISP_SUBDEV_TYPE_SOURCE) {
            ...
            if (ret < 0) {
                pr_info("Failed to initialize source subdev %s: %d\n",
    ...
    for (i = 0; i < subdev_count; i++) {
        ...
        if (runtime->desc->type == TX_ISP_SUBDEV_TYPE_SINK) {
            ...
            if (ret < 0) {
                pr_info("Failed to initialize sink subdev %s: %d\n",
                       runtime->desc->name, ret);
                goto cleanup;
            }
        }
...
cleanup:
    pr_info("tx_isp_create_subdev_graph: Graph creation failed, cleaning up\n");
                                    ...
                                    struct tx_isp_subdev_runtime *runtime)
{
    ...
    if (!driver_data) {
        pr_info("tx_isp_init_source_subdev: No driver data for %s\n", desc->name);
        return 0;
    }
                                  ...
                                  struct tx_isp_subdev_runtime *runtime)
{
    ...
    if (!driver_data) {
        pr_info("tx_isp_init_sink_subdev: No driver data for %s\n", desc->name);
        return 0;
    }

    /* Find source subdevice */
    if (desc->src_index >= ISP_MAX_SUBDEVS) {
        pr_info("tx_isp_init_sink_subdev: Invalid source index %d for %s\n",
    ...
    if (!src_subdev) {
        pr_info("tx_isp_init_sink_subdev: Source subdev %d not found for %s\n",
               desc->src_index, desc->name);
        return -ENODEV;
    }
    ...
    if (ret < 0) {
        pr_info("tx_isp_init_sink_subdev: Failed to create link for %s: %d\n",
               desc->name, ret);
        return ret;
    }
...
static int tx_isp_create_misc_device(struct tx_isp_subdev_runtime *runtime)
{
    ...
    if (ret < 0) {
        pr_info("Failed to register misc device %s: %d\n",
               runtime->desc->name, ret);
        kfree(misc_dev->name);
        kfree(misc_dev);
        return ret;
...
static int tx_isp_csi_device_init(struct tx_isp_dev *isp)
{
    ...
    if (!isp->csi_dev) {
        ...
        if (!csi_dev) {
            pr_info("Failed to allocate CSI device\n");
            return -ENOMEM;
        }
...
static int tx_isp_create_basic_pipeline(struct tx_isp_dev *isp)
{
    ...
    if (ret < 0) {
        pr_info("Failed to initialize CSI device: %d\n", ret);
        return ret;
    }
    ...
    if (ret < 0) {
        pr_info("Failed to initialize VIC device: %d\n", ret);
    ...
    if (ret < 0) {
        pr_info("Failed to setup pipeline: %d\n", ret);
                               ...
                               int count)
{
    ...
    if (!isp || !platform_devices) {
        pr_info("tx_isp_init_subdev_registry: Invalid parameters\n");
        return -EINVAL;
    }
    ...
    for (i = 0; i < count && i < NUM_ISP_SUBDEVS; i++) {
        ...
        if (!driver_data) {
            pr_info("Failed to create driver data for %s\n", desc->name);
            continue;
        }

        ret = tx_isp_subdev_register(desc, platform_devices[i], driver_data);
        if (ret < 0) {
            pr_info("Failed to register subdevice %s: %d\n", desc->name, ret);
            kfree(driver_data);
            continue;
        }
...
{
    if (!isp) {
        pr_info("tx_isp_csi_device_deinit: Invalid ISP device\n");
        return -EINVAL;
    }
...
{
    if (!isp) {
        pr_info("tx_isp_vic_device_deinit: Invalid ISP device\n");
        return -EINVAL;
    }
