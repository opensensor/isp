...
static int ispvic_frame_channel_qbuf(void *arg1, void *arg2)
{
    ...
    /* Binary Ninja EXACT: else if ($s0 + 0x1f4 == *($s0 + 0x1f4)) */
        ...
        pr_info("*** ispvic_frame_channel_qbuf: Processing queued buffer ***\n");
    }
...
/* VIC interrupt service routine - SAFE implementation with comprehensive list validation */
irqreturn_t isp_vic_interrupt_service_routine(int irq, void *dev_id)
{
    ...
    pr_info("*** VIC IRQ: Processing interrupt for vic_dev=%p ***\n", vic_dev);
    ...
    if (list_empty(&vic_dev->queue_head) || list_empty(&vic_dev->free_head)) {
        pr_info("*** VIC IRQ: Lists empty - queue_empty=%d, free_empty=%d ***\n",
    ...
    /* Process the buffer */
    buffer_addr = queue_buffer->buffer_addr;
    if (buffer_addr == 0) {
        buffer_addr = 0x30000000 + (vic_dev->active_buffer_count * 0x100000);
    }

    buffer_index = vic_dev->active_buffer_count % 5;
    free_buffer->buffer_addr = buffer_addr;

    /* Program VIC hardware register */
    if (vic_dev->vic_regs) {
    ...
    /* Add to done list */
    list_add_tail(&free_buffer->list, &vic_dev->done_head);
    vic_dev->active_buffer_count += 1;

    spin_unlock_irqrestore(&vic_dev->buffer_mgmt_lock, flags);

    pr_info("*** VIC IRQ: Buffer processing complete - active_count=%d ***\n",
            vic_dev->active_buffer_count);
