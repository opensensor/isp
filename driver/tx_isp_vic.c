    ...
    if (enable == 0) {
        ...
        if (current_state != 4) {
            ...
            if (vic_dev->vic_regs) {
				...
				pr_info("Re-assert interrupt mask in BOTH banks");
                /* Clear pending (W1C) */
            ...
            if (vic_dev && vic_dev->vic_regs) {
                ...
                if (vic_dev && vic_dev->vic_regs && vic_dev->state < 2) {
...
static struct tx_isp_subdev_video_ops vic_video_ops = {
    ...
    .link_stream = NULL,  /* CRITICAL FIX: tx_isp_video_link_stream calls link_stream! */
};
