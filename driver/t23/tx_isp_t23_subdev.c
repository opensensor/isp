...
static void *tx_isp_t23_remote_pad(void *local_pad)
{
	return *(void **)((char *)local_pad + TX_ISP_ABI_PAD_LINK_OFFSET +
			 TX_ISP_ABI_LINK_SINK_OFFSET);
}

static unsigned long tx_isp_t23_event_handler(void *remote_pad)
{
	return *(unsigned int *)((char *)remote_pad +
				TX_ISP_ABI_PAD_EVENT_OFFSET);
}

static const struct tx_isp_remote_event_ops tx_isp_t23_remote_event_ops = {
	.remote_pad = tx_isp_t23_remote_pad,
	.event_handler = tx_isp_t23_event_handler,
};

enum tx_isp_remote_event_status tx_isp_t23_resolve_remote_event(
	void *local_pad,
