...
/* CRITICAL FIX: Remove all conflicting redefinitions */
/* Use the correct enum values from tx-isp-common.h: MIPI=1, DVP=2 */
/* CRITICAL FIX: Use correct enum values from tx-isp-common.h */
/* TX_SENSOR_DATA_INTERFACE_MIPI = 1, TX_SENSOR_DATA_INTERFACE_DVP = 2 */
/* The sensor driver was incorrectly swapping these values, causing VIC to configure for wrong interface */
#define TX_SENSOR_DATA_INTERFACE_MIPI 1
#define TX_SENSOR_DATA_INTERFACE_DVP 2
...
static int sensor_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	*/
	...
	ISP_WARNING("sensor_probe: BEFORE set_subdevdata: sd=%p sd->dev_priv=%p sd->host_priv=%p\n",
	            sd, sd->dev_priv, sd->host_priv);
	tx_isp_set_subdevdata(sd, client);
	ISP_WARNING("sensor_probe: AFTER set_subdevdata: sd->dev_priv=%p (should be client=%p)\n",
	            sd->dev_priv, client);
	tx_isp_set_subdev_hostdata(sd, sensor);
	ISP_WARNING("sensor_probe: AFTER set_subdev_hostdata: sd->host_priv=%p (should be sensor=%p)\n",
	            sd->host_priv, sensor);
	private_i2c_set_clientdata(client, sd);
	ISP_WARNING("sensor_probe: AFTER i2c_set_clientdata: i2c_get_clientdata=%p (should be sd=%p)\n",
	            private_i2c_get_clientdata(client), sd);
