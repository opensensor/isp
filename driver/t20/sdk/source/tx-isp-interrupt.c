...
static void tx_isp_disable_irq(struct tx_isp_irq_device *irq_dev, int enable)
{
	...
	tx_isp_writel(irq_dev->base, TX_ISP_TOP_IRQ_ENABLE, reg);
	/*
	 * APICAL uses this operation as a short firmware critical section.
	 * Synchronous disable_irq() is both unnecessary after the top source is
	 * masked and unsafe while the SDK spinlock is held, especially with the
	 * T20 threaded ISP handler.  Keep Linux IRQ ownership unchanged; teardown
	 * synchronizes it through the normal free_irq path.
	 */
	spin_unlock_irqrestore(&irq_dev->slock, flags);
}
