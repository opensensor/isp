		...
		struct v4l2_format *f)
{
	...
	v4l_bound_align_image(&f->fmt.pix.width,
			...
			&f->fmt.pix.height,
			attr->min_height, attr->max_height, 1, 0);
...
//		return ret;
	...
	/* The private IMP ABI programs DS scaling with a separate ioctl.  Public
	 * V4L2 applications reasonably expect S_FMT to be sufficient, so bridge
	 * the requested DS dimensions into that already-proven scaler operation
	 * before the core verifies the output geometry. */
	if (vdev->index != 0 &&
	    (f->fmt.pix.width != attr->output.fmt.pix.width ||
	     f->fmt.pix.height != attr->output.fmt.pix.height)) {
		if (f->fmt.pix.width < attr->min_width ||
		    f->fmt.pix.width > attr->max_width ||
		    f->fmt.pix.height < attr->min_height ||
		    f->fmt.pix.height > attr->max_height)
			return -EINVAL;
		attr->scaler.out_width = f->fmt.pix.width;
		attr->scaler.out_height = f->fmt.pix.height;
...
static int frame_channel_vidioc_expbuf(struct file *file, void *priv,
		struct v4l2_exportbuffer *p)
{
	frame_chan_vdev_t *vdev = video_drvdata(file);

	return vb2_expbuf(&vdev->vbq, p);
}
...
static const struct v4l2_ioctl_ops frame_channel_v4l2_ioctl_ops = {
	...
	.vidioc_expbuf			= frame_channel_vidioc_expbuf,
