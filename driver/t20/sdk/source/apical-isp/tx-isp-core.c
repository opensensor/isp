...
module_param(t20_irq_allow_mask, uint, 0644);
MODULE_PARM_DESC(t20_irq_allow_mask,
	"temporary T20 bring-up mask for ISP-core interrupt dispatch");
static unsigned int t20_frame_end_stage = 9;
module_param(t20_frame_end_stage, uint, 0644);
MODULE_PARM_DESC(t20_frame_end_stage,
	"temporary T20 bring-up stage gate for frame-end IRQ processing");
...
void system_hw_interrupts_enable(void)
{
	...
	/*
	 * The primary handler already owns the line in hard-IRQ context.  In
	 * process context the T20 interrupt device masks/unmasks the ISP top-level
	...
	if (!use_to_intc_sd || !use_to_intc_sd->v4l2_dev)
		return;
...
void system_hw_interrupts_disable(void)
{
	...
	if (!use_to_intc_sd || !use_to_intc_sd->v4l2_dev)
		return;
	arg.value = TX_ISP_TOP_IRQ_ISP;
	use_to_intc_sd->v4l2_dev->notify(use_to_intc_sd,
		TX_ISP_NOTIFY_DISABLE_IRQ, &arg);
}
...
static int isp_core_interrupt_service_routine(struct v4l2_subdev *sd, u32 status, bool *handled)
{
	...
	if ((isp_irq_status = isp_intc_state()) != 0) {
		...
		isp_irq_status &= t20_irq_allow_mask;
			...
			if (isp_irq_status & (1 << i)) {
				...
				switch (i) {
					...
					case APICAL_IRQ_FRAME_END:
						...
						if (t20_frame_end_stage == 1)
							break;
						/* APICAL_WRITE_32(0x18,2);  */
						core->frame_state = 0;
						if (t20_frame_end_stage == 2)
							break;
						isp_configure_base_addr(core);
						if (t20_frame_end_stage == 3)
							break;
						#if TX_ISP_EXIST_FR_CHANNEL
						chan = &core->chans[ISP_FR_VIDEO_CHANNEL];
						isp_modify_dma_direction(chan);
						if (t20_frame_end_stage == 4)
							break;
						if (chan->dma_state != 1) {
							isp_enable_channel(chan);
						}
						#endif
						if (t20_frame_end_stage == 5)
							break;

						isp_frame_done_wakeup();
						if (t20_frame_end_stage == 6)
							break;

						if (1 == core->isp_daynight_switch) {
							core->isp_daynight_switch = 0;
							schedule_work(&core->daynight_work);
						}
						if (t20_frame_end_stage == 7)
							break;
...
{
	static unsigned int t20_i2c_thread_trace_count;
	...
	if (core) {
		if (t20_i2c_thread_trace_count < 24 &&
			...
			printk(KERN_INFO
			       "T20I2C thread n=%u core=%p flags=%u/%u/%u values=%u/%u/%u\n",
			       t20_i2c_thread_trace_count, core,
			       core->i2c_msgs[0].flag, core->i2c_msgs[1].flag,
			       core->i2c_msgs[2].flag, core->i2c_msgs[0].value,
			       core->i2c_msgs[1].value, core->i2c_msgs[2].value);
...
static int inline isp_core_video_streamon(struct tx_isp_core_device *core)
{
	...
	api.dir = COMMAND_SET;
	api.value = PAUSE;
	printk(KERN_INFO "T20STREAM pause begin hw=%u\n",
		APICAL_READ_32(0x130) & 7);
	status = apical_command(api.type, api.id, api.value, api.dir, &ret);
	printk(KERN_INFO "T20STREAM pause end status=%u reason=%d hw=%u\n",
		status, ret, APICAL_READ_32(0x130) & 7);
	...
	api.dir = COMMAND_SET;
	api.value = RUN;
	printk(KERN_INFO "T20STREAM run begin hw=%u irq_enable=0x%x irq_mask=0x%x\n",
		APICAL_READ_32(0x130) & 7,
		APICAL_READ_32(0x14), APICAL_READ_32(0x1c));
	status = apical_command(api.type, api.id, api.value, api.dir, &reason);
	printk(KERN_INFO "T20STREAM run end status=%u reason=%u hw=%u\n",
		status, reason, APICAL_READ_32(0x130) & 7);
...
int register_tx_isp_core_device(struct platform_device *pdev, struct v4l2_device *v4l2_dev)
{
	...
	INIT_WORK(&core_dev->daynight_work, t20_daynight_work);
	core_dev->refcnt = 0;
