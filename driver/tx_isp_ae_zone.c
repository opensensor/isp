...
{
    printk(KERN_ALERT "MCP_LOG: %s: %s, data=%p\n", method_name, description, data);
}
...
int tisp_ae_get_y_zone(void *arg1)
{
    ...
    if (!arg1) {
        printk(KERN_ALERT "tisp_ae_get_y_zone: Invalid argument\n");
...
int tisp_g_ae_zone(struct tx_isp_dev *dev, struct isp_core_ctrl *ctrl)
{
    ...
    if (!dev || !ctrl) {
        printk(KERN_ALERT "tisp_g_ae_zone: Invalid device or control pointer\n");
        mcp_log_info("tisp_g_ae_zone", "error - invalid parameters", NULL);
        return -EINVAL;
    }

    if (!ctrl->value) {
        printk(KERN_ALERT "tisp_g_ae_zone: No data pointer for AE zone\n");
    ...
    if (ret) {
        printk(KERN_ALERT "tisp_g_ae_zone: Failed to get AE zone data: %d\n", ret);
    ...
    if (copy_to_user((void __user *)ctrl->value, &zones, sizeof(zones))) {
        printk(KERN_ALERT "tisp_g_ae_zone: Failed to copy data to user\n");
...
int tisp_ae_update_zone_data(uint32_t *new_zone_data, size_t data_size)
{
    ...
    if (!new_zone_data || data_size != sizeof(ae_zone_data.zone_data)) {
        printk(KERN_ALERT "tisp_ae_update_zone_data: Invalid parameters\n");
