                      ...
                      struct tx_isp_subdev_ops *ops)
{
    ...
    if (pdata != NULL) {
        /* CRITICAL: Skip IRQ request for CSI and VIC - defer VIC IRQ until hardware is ready */
        const char *dev_name_str = dev_name(&pdev->dev);
        if (strcmp(dev_name_str, "tx-isp-csi") != 0 && strcmp(dev_name_str, "isp-w02") != 0) {
            ...
            pr_info("*** tx_isp_subdev_init: VIC IRQ registration DEFERRED until hardware ready ***\n");
            /* Initialize IRQ info but don't register yet */
            sd->irq_info.irq = 0;
