                      ...
                      struct tx_isp_subdev_ops *ops)
{
    ...
    if (ourISPdev) {
        if (ops == &csi_subdev_ops) {
            /* CSI - register at slot 0 */
            if (ourISPdev->subdevs[0] == NULL) {
                ourISPdev->subdevs[0] = sd;
                sd->isp = ourISPdev;
                pr_info("*** tx_isp_subdev_init: CSI subdev registered at slot 0 ***\n");
            } else {
                pr_err("*** tx_isp_subdev_init: CSI slot 0 already occupied ***\n");
            }
        } else if (ops == &vic_subdev_ops) {
            /* VIC - register at slot 1 and link VIC device */
            struct tx_isp_vic_device *vic_dev = container_of(sd, struct tx_isp_vic_device, sd);
            ourISPdev->vic_dev = vic_dev;
            if (ourISPdev->subdevs[1] == NULL) {
                ourISPdev->subdevs[1] = sd;
                sd->isp = ourISPdev;
                pr_info("*** tx_isp_subdev_init: VIC device linked and registered at slot 1 ***\n");
            } else {
                pr_err("*** tx_isp_subdev_init: VIC slot 1 already occupied ***\n");
            }
        } else if (ops == &core_subdev_ops) {
            /* CORE - register at slot 4 */
            if (ourISPdev->subdevs[4] == NULL) {
                ourISPdev->subdevs[4] = sd;
                sd->isp = ourISPdev;
                pr_info("*** tx_isp_subdev_init: Core ISP subdev registered at slot 4 ***\n");
            } else {
                pr_err("*** tx_isp_subdev_init: Core slot 4 already occupied ***\n");
            }
