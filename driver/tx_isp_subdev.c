...
int isp_subdev_init_clks(struct tx_isp_subdev *sd, int clk_count)
{
    ...
    if (sd->pdev && sd->pdev->dev.platform_data) {
        ...
        pdata = NULL;
        clk_configs = NULL;
        printk(KERN_ALERT "isp_subdev_init_clks: No platform data - using fallback clocks\n");
    }
    ...
    if (clk_count != 0) {
        ...
        if (clk_configs) {
            ...
            printk(KERN_ALERT "isp_subdev_init_clks: No platform data clock configs - using fallback\n");
        }
        ...
        while (i < clk_count) {
            ...
            if (clk_configs && i < clk_count) {
                ...
                if (i < 3) {
                    ...
                    printk(KERN_ALERT "Using fallback clock[%d]: name=%s, rate=%lu\n", i, clk_name, clk_rate);
                } else {
            ...
            if (!IS_ERR_OR_NULL(clk)) {
                ...
                if (clk_rate == 0xffff || result == 0) {
                    ...
                    if (ret == 0) {
                        ...
                        printk(KERN_ALERT "Failed to enable clock %s, continuing anyway\n", clk_name);
                        i++;
                        continue;
                    }
                } else {
                    /* Binary Ninja: isp_printf(2, "sensor type is BT1120!\n", *$s6_1) */
                    printk(KERN_ALERT "Failed to set rate for clock %s, continuing anyway\n", clk_name);
                    /* Binary Ninja: $s0_3 = $s0_2 << 2 - goto cleanup */
                    break;
                }
            } else {
                /* Binary Ninja: isp_printf(2, "Can not support this frame mode!!!\n", *$s6_1) */
                printk(KERN_ALERT "Failed to get clock %s\n", clk_name);
    ...
    if (ourISPdev) {
        if (ops == &csi_subdev_ops) {
            ...
            if (slot >= 0) {
                ...
                printk(KERN_ALERT "*** tx_isp_subdev_init: No available slot for sensor subdev ***\n");
            }
        } else {
    ...
    if (pdata != NULL) {
        ...
        if (mem_res) {
            ...
            if (!sd->mem_res) {
                ...
                printk(KERN_ALERT "tx_isp_subdev_init: request_mem_region failed for %s (0x%08x-0x%08x)\n",
...
void tx_isp_subdev_auto_link(struct platform_device *pdev, struct tx_isp_subdev *sd)
{
    ...
    if (!ourISPdev) {
        printk(KERN_ALERT "*** CRITICAL: tx_isp_subdev_auto_link: ourISPdev is NULL! ***\n");
        return;
    }
    if (!pdev) {
        printk(KERN_ALERT "*** CRITICAL: tx_isp_subdev_auto_link: pdev is NULL! ***\n");
        return;
    }
    if (!pdev->name) {
        printk(KERN_ALERT "*** CRITICAL: tx_isp_subdev_auto_link: pdev->name is NULL! ***\n");
        return;
    }
    if (!sd) {
        printk(KERN_ALERT "*** CRITICAL: tx_isp_subdev_auto_link: sd is NULL! ***\n");
        return;
    }
    ...
    if (strcmp(dev_name, "isp-w00") == 0) {
        ...
        if (!vic_dev || (unsigned long)vic_dev < 0x80000000 || (unsigned long)vic_dev >= 0xfffff000) {
            printk(KERN_ALERT "*** VIC DEVICE LINKING FAILED: Invalid vic_dev pointer %p ***\n", vic_dev);
            return;
        }
        ...
        if (vic_dev->sd.irq_info.irq == 0) {
            ...
            if (irq_ret != 0) {
                printk(KERN_ALERT "*** VIC AUTO-LINK: Failed to register VIC IRQ: %d ***\n", irq_ret);
            } else {
        ...
        if (sd->regs && ourISPdev->vic_dev && ourISPdev->vic_dev->vic_regs) {
            ...
            printk(KERN_ALERT "*** VIC AUTO-LINK: Registers not mapped - cannot register interrupt ***\n");
        }
        ...
        if (slot >= 0) {
            ...
            printk(KERN_ALERT "*** Failed to register VIN subdev - no available slots ***\n");
        }
        ...
        if (slot >= 0) {
            ...
            printk(KERN_ALERT "*** Failed to register FS subdev - no available slots ***\n");
        }
        ...
        if (core_dev) {
            ...
            if (link_ret != 0) {
                printk(KERN_ALERT "*** CORE AUTO-LINK: Failed to link core device: %d ***\n", link_ret);
            } else {
                printk(KERN_ALERT "*** LINKED CORE device: %p ***\n", core_dev);
                printk(KERN_ALERT "*** CORE SUBDEV REGISTERED AT INDEX 0 ***\n");
            }
        } else {
            printk(KERN_ALERT "*** CRITICAL ERROR: Core device not found in subdev private data ***\n");
        }
        ...
        if (!already_registered) {
            ...
            if (sensor_index != -1) {
                ...
                printk(KERN_ALERT "*** No available slot for sensor '%s' ***\n", dev_name);
            }
        }
    } else {
...
int tx_isp_reg_set(struct tx_isp_subdev *sd, unsigned int reg, int start, int end, int val)
{
    ...
    if (!sd || !sd->regs) {
        printk(KERN_ALERT "tx_isp_reg_set: Invalid subdev or registers not mapped");
        return -EINVAL;
    }
    ...
    *reg_addr = (val << (start & 0x1f)) | ((~mask) & *reg_addr);
    ...
    printk(KERN_ALERT "tx_isp_reg_set: subdev=%p reg[0x%x] = 0x%x (bits %d-%d = %d)",
...
{
    if (!pdev || !sd) {
        printk(KERN_ALERT "tx_isp_module_init: Invalid parameters\n");
        return -EINVAL;
    }
...
{
    if (!sd) {
        printk(KERN_ALERT "tx_isp_module_deinit: Invalid subdev\n");
        return;
    }
...
int tx_isp_module_notify_handler(struct tx_isp_module *module, unsigned int cmd, void *arg)
{
    ...
    if (!module) {
        printk(KERN_ALERT "tx_isp_module_notify_handler: Invalid module\n");
        return -EINVAL;
    }
    ...
    switch (cmd) {
        case TX_ISP_EVENT_SYNC_SENSOR_ATTR:
            ...
            if (video && video->attr) {
                ...
                printk(KERN_ALERT "*** tx_isp_module_notify_handler: No video attributes available ***\n");
                return -EINVAL;
            }
...
int tx_isp_request_irq(struct platform_device *pdev, struct tx_isp_irq_info *irq_info)
{
    ...
    if (irq_num >= 0) {
        ...
        if (ret != 0) {
            ...
            printk(KERN_ALERT "*** CRITICAL: tx_isp_request_irq: request_threaded_irq FAILED for IRQ %d: %d ***\n", irq_num, ret);
...
void tx_isp_free_irq(struct tx_isp_irq_info *irq_info)
{
    if (!irq_info || irq_info->irq <= 0) {
        printk(KERN_ALERT "tx_isp_free_irq: Invalid IRQ info\n");
        return;
    }
...
int __init tx_isp_subdev_platform_init(void)
{
    ...
    if (ret) {
        printk(KERN_ALERT "Failed to register VIC platform driver: %d\n", ret);
    ...
    if (ret) {
        printk(KERN_ALERT "Failed to register CSI platform driver: %d\n", ret);
        return ret;
    }
    ...
    if (ret) {
        printk(KERN_ALERT "Failed to register VIN platform driver: %d\n", ret);
    ...
    if (ret) {
        printk(KERN_ALERT "Failed to register CORE platform driver: %d\n", ret);
