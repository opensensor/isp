...
static inline int t41_awb_surface(const unsigned char *p, unsigned int pb,
		unsigned int map_offset, unsigned int red, unsigned int blue, unsigned int *out)
{
	unsigned int precision, fraction, x[15], y[15], ix=0, iy=0, i, a, b, mired;
	const unsigned char *map;
	if (!p || pb<0xcd6 || !out || map_offset>pb || pb-map_offset<900) return -1;
...
{
	unsigned int mired, fraction;
	if (!ct || t41_awb_surface(p,pb,0x870,red,blue,&mired)) return -1;
	fraction=t41_tmo_le16(p+0xcd4);
