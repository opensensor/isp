...
int tx_isp_vic_start_streaming(struct tx_isp_dev *isp_dev)
{
    ...
    printk(KERN_ALERT "*** tx_isp_vic_start_streaming: Starting VIC streaming %dx%d ***\n", width, height);
    ...
    printk(KERN_ALERT "*** tx_isp_vic_start_streaming: VIC streaming started successfully ***\n");
    printk(KERN_ALERT "*** vic_start_ok = %d, interrupts enabled ***\n", vic_start_ok);
...
int tx_isp_vic_stop_streaming(struct tx_isp_dev *isp_dev)
{
    ...
    printk(KERN_ALERT "*** tx_isp_vic_stop_streaming: Stopping VIC streaming ***\n");
    ...
    printk(KERN_ALERT "*** KEEPING vic_start_ok=1 to preserve interrupt protection during restart ***\n");
    ...
    printk(KERN_ALERT "*** SKIPPING VIC hardware disable (register 0x0) to preserve interrupt configuration ***\n");
    ...
    printk(KERN_ALERT "*** tx_isp_vic_stop_streaming: VIC streaming stopped ***\n");
...
int tx_isp_mipi_phy_status_check(struct tx_isp_dev *isp_dev)
{
    ...
    printk(KERN_ALERT "*** tx_isp_mipi_phy_status_check: Checking MIPI PHY status ***\n");
    
    /* Check clock lane status */
    clock_status = readl(phy_regs + 0x14);
    printk(KERN_ALERT "MIPI Clock Lane Status: 0x%08x\n", clock_status);
    
    if (clock_status & 0x1) {
        printk(KERN_ALERT "  - Clock lane: ACTIVE\n");
    } else {
    ...
    for (i = 0; i < 4; i++) {
        ...
        printk(KERN_ALERT "MIPI Data Lane %d Status: 0x%08x\n", i, lane_status);
        
        if (lane_status & 0x1) {
            printk(KERN_ALERT "  - Data lane %d: SYNC\n", i);
        } else {
            pr_warn("  - Data lane %d: NO SYNC\n", i);
        }
        
        if (lane_status & 0x2) {
            printk(KERN_ALERT "  - Data lane %d: RECEIVING DATA\n", i);
        } else {
    ...
    printk(KERN_ALERT "MIPI PHY Error Status: 0x%08x\n", error_status);
    ...
    printk(KERN_ALERT "MIPI PHY Configuration: 0x%08x\n", phy_config);
    printk(KERN_ALERT "  - PHY frequency setting: %d\n", phy_config & 0xf);
...
void tx_isp_vic_register_dump(struct tx_isp_dev *isp_dev)
{
    ...
    printk(KERN_ALERT "*** VIC Register Dump ***\n");
    printk(KERN_ALERT "VIC_CTRL (0x00):     0x%08x\n", readl(vic_regs + 0x0));
    printk(KERN_ALERT "VIC_STATUS (0x04):   0x%08x\n", readl(vic_regs + 0x4));
    printk(KERN_ALERT "VIC_CONFIG (0x0C):   0x%08x\n", readl(vic_regs + 0xc));
    printk(KERN_ALERT "VIC_SIZE (0x10):     0x%08x\n", readl(vic_regs + 0x10));
    printk(KERN_ALERT "VIC_STRIDE (0x14):   0x%08x\n", readl(vic_regs + 0x14));
    printk(KERN_ALERT "VIC_INT_EN (0x1E0):  0x%08x\n", readl(vic_regs + 0x1e0));
    printk(KERN_ALERT "VIC_INT_MASK (0x1E8): 0x%08x\n", readl(vic_regs + 0x1e8));
    printk(KERN_ALERT "VIC_INT_STAT (0x1F0): 0x%08x\n", readl(vic_regs + 0x1f0));
    printk(KERN_ALERT "VIC_BUFFER0 (0x380): 0x%08x\n", readl(vic_regs + 0x380));
    printk(KERN_ALERT "VIC_BUFFER1 (0x384): 0x%08x\n", readl(vic_regs + 0x384));
    printk(KERN_ALERT "VIC_BUFFER2 (0x388): 0x%08x\n", readl(vic_regs + 0x388));
    printk(KERN_ALERT "*** End VIC Register Dump ***\n");
}
...
void tx_isp_debug_frame_capture_status(struct tx_isp_dev *isp_dev)
{
    ...
    printk(KERN_ALERT "*** Frame Capture Status Debug ***\n");
    printk(KERN_ALERT "Driver frame count: %u\n", isp_dev->frame_count);
    printk(KERN_ALERT "VIC start flag: %d\n", vic_start_ok);
    
    /* Check hardware frame counter if available */
    frame_count_reg = readl(vic_regs + 0x160);
    printk(KERN_ALERT "Hardware frame count: %u\n", frame_count_reg);
    
    /* Check buffer status */
    buffer_status = readl(vic_regs + 0x300);
    printk(KERN_ALERT "Buffer status: 0x%08x\n", buffer_status);
    
    /* Check if VIC is actually receiving data */
    uint32_t vic_status = readl(vic_regs + 0x4);
    printk(KERN_ALERT "VIC status register: 0x%08x\n", vic_status);
    
    if (vic_status & 0x1) {
        printk(KERN_ALERT "  - VIC is ACTIVE\n");
    } else {
        pr_warn("  - VIC is INACTIVE - No data flow!\n");
    }
    
    if (vic_status & 0x2) {
        printk(KERN_ALERT "  - Frame sync detected\n");
    } else {
        pr_warn("  - No frame sync - Check sensor output!\n");
    }
    
    /* Error counters */
    printk(KERN_ALERT "*** Error Counters ***\n");
    printk(KERN_ALERT "Control limit errors: %u\n", data_b2984);
    printk(KERN_ALERT "Frame FIFO overflows: %u\n", data_b299c);
    printk(KERN_ALERT "DMA FIFO overflows: %u\n", data_b2980);
    printk(KERN_ALERT "MIPI errors: %u\n", data_b2990 + data_b2994);
    
    printk(KERN_ALERT "*** End Frame Capture Status ***\n");
}
