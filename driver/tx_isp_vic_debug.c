...
int tx_isp_vic_start_streaming(struct tx_isp_dev *isp_dev)
{
    ...
    if (!isp_dev || !isp_dev->vic_regs) {
        pr_info("tx_isp_vic_start_streaming: Invalid parameters\n");
        return -EINVAL;
    }
    ...
    if (width == 0 || height == 0 || width > 8192 || height > 8192) {
        pr_info("*** tx_isp_vic_start_streaming: INVALID DIMENSIONS %dx%d ***\n", width, height);
        pr_info("*** This will cause VIC control limit error! ***\n");
        return -EINVAL;
    }
    ...
    if (buffer_size > 0x2000000) { /* 32MB limit */
        pr_info("*** tx_isp_vic_start_streaming: Buffer size too large: %d bytes ***\n", buffer_size);
        return -EINVAL;
    }
...
int tx_isp_vic_stop_streaming(struct tx_isp_dev *isp_dev)
{
    ...
    if (!isp_dev || !isp_dev->vic_regs) {
        pr_info("tx_isp_vic_stop_streaming: Invalid parameters\n");
        return -EINVAL;
    }
...
int tx_isp_mipi_phy_status_check(struct tx_isp_dev *isp_dev)
{
    ...
    if (!isp_dev || !isp_dev->phy_base) {
        pr_info("tx_isp_mipi_phy_status_check: Invalid parameters\n");
        return -EINVAL;
    }
    ...
    if (clock_status & 0x1) {
        ...
        pr_info("  - Clock lane: INACTIVE - No clock detected!\n");
    }
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (lane_status & 0x1) {
            ...
            pr_info("  - Data lane %d: NO SYNC\n", i);
        }
        
        if (lane_status & 0x2) {
            pr_info("  - Data lane %d: RECEIVING DATA\n", i);
        } else {
            pr_info("  - Data lane %d: NO DATA\n", i);
        }
    }
    ...
    if (error_status & 0x1) {
        pr_info("  - PHY Error: Clock lane error\n");
    }
    if (error_status & 0x2) {
        pr_info("  - PHY Error: Data lane 0 error\n");
    }
    if (error_status & 0x4) {
        pr_info("  - PHY Error: Data lane 1 error\n");
    }
    if (error_status & 0x8) {
        pr_info("  - PHY Error: Escape mode error\n");
    }
...
void tx_isp_vic_register_dump(struct tx_isp_dev *isp_dev)
{
    ...
    if (!isp_dev || !isp_dev->vic_regs) {
        pr_info("tx_isp_vic_register_dump: Invalid parameters\n");
        return;
    }
...
void tx_isp_debug_frame_capture_status(struct tx_isp_dev *isp_dev)
{
    ...
    if (!isp_dev || !isp_dev->vic_regs) {
        pr_info("tx_isp_debug_frame_capture_status: Invalid parameters\n");
        return;
    }
    ...
    if (vic_status & 0x1) {
        ...
        pr_info("  - VIC is INACTIVE - No data flow!\n");
    }
    
    if (vic_status & 0x2) {
        pr_info("  - Frame sync detected\n");
    } else {
        pr_info("  - No frame sync - Check sensor output!\n");
    }
