...
static unsigned char p[T41_CDNS_PARAM_BYTES], actual[T41_CDNS_STATE_BYTES], expected[sizeof(actual)];
...
int main(void)
{
	...
	*(uint32_t *)(void *)(oracle_bss+0x46e0)=(uintptr_t)info;
	*(uint32_t *)(void *)(oracle_bss+0x46e4)=(uintptr_t)info;
	...
	for(f=0;f<10000;++f) {
		...
		if(t41_cdns_interpolate(p,sizeof(p),actual,sizeof(actual),gain)) return 2;
		oracle_interpolate(channel,gain);
		for(i=0;i<sizeof(actual);++i) if(actual[i]!=expected[i]) {
			if(failures++<12) printf("CDNS interp frame=%u +%x got=%x OEM=%x\n",f,i,actual[i],expected[i]);
			break;
		}
		if(f&2) { for(i=0;i<sizeof(actual);++i) actual[i]=rng(); memcpy(expected,actual,sizeof(actual)); }
		count=t41_cdns_pack(p,sizeof(p),actual,sizeof(actual),channel,words,T41_CDNS_WRITES);
		...
		for(i=0;i<oracle_count;++i) if(words[i].address!=oracle_addresses[i] || words[i].value!=oracle_values[i]) {
			if(failures++<12) printf("CDNS write frame=%u word=%u %x=%x OEM=%x=%x\n",
				f,i,words[i].address,words[i].value,oracle_addresses[i],oracle_values[i]);
		}
	}
	for(f=0;f<65536;++f) {
		struct t41_dpc_word words[T41_CDNS_WRITES];
		actual[30]=f&255; actual[31]=f>>8;
		memcpy(expected,actual,sizeof(actual));
		if(t41_cdns_pack(p,sizeof(p),actual,sizeof(actual),0,words,T41_CDNS_WRITES)!=12) return 3;
		oracle_count=0; oracle_pack(0);
		if(words[11].value!=oracle_values[11]) {
			if(failures++<12) printf("CDNS signed threshold pair=%04x got=%x OEM=%x\n",f,words[11].value,oracle_values[11]);
		}
	}
	printf("10000 synthetic CDNS interpolations and register transactions, 65536 signed threshold pairs: %u mismatches, %u unexpected writes\n",failures,oracle_bad_write);
